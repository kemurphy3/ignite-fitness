...
class Prompt22Verification {
  ...
  testProperDisclaimers() {
    ...
    if (lc.disclaimers && lc.disclaimers.injuryAssessment) {
      ...
      const hasNoDiagnosis =
        disclaimer.includes('not medical advice') || disclaimer.includes('not a substitute');
      const hasConsult =
        disclaimer.includes('healthcare professional') || disclaimer.includes('consult');
      const hasEducational =
        disclaimer.includes('educational') || disclaimer.includes('exercise modifications');
  ...
  testBiomechanicallySound() {
    ...
    if (kneeMods) {
      ...
      if (
        kneeMods.substitute &&
        kneeMods.substitute.some(
          exercise => exercise.name && exercise.name.toLowerCase().includes('goblet')
        )
      ) {
    ...
    if (lbMods) {
      ...
      if (
        lbMods.substitute &&
        lbMods.substitute.some(
          exercise => exercise.name && exercise.name.toLowerCase().includes('trap')
        )
      ) {
  ...
  testLegalCompliance() {
    ...
    // Test timestamping
    if (
      typeof lc.timestampDisclaimer === 'function' ||
    ...
    if (
      lc.disclaimers &&
      lc.disclaimers.injuryAssessment.text.includes('healthcare professional')
    ) {
