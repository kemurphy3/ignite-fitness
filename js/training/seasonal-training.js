...
class SeasonalTrainingSystem {
  ...
  generatePhaseWorkoutPlan(baseWorkout) {
    ...
    const adjustedWorkout = {
      ...
      sessions: baseWorkout.sessions.map(session => ({
        ...session,
        duration: Math.min(
          session.duration * adjustments.volumeMultiplier,
  ...
  addGame(game) {
    try {
      ...
      const existingGame = this.gameSchedule.find(
        g => g.date.toDateString() === gameDate.toDateString()
      );
  ...
  getUpcomingGames(count = 5) {
    const now = new Date();
    return this.gameSchedule.filter(game => game.date >= now).slice(0, count);
  }
  ...
  getGamesThisWeek() {
    ...
    return this.gameSchedule.filter(game => game.date >= weekStart && game.date <= weekEnd);
  }

  // Check if it's a game day
  isGameDay(date = new Date()) {
    return this.gameSchedule.some(game => game.date.toDateString() === date.toDateString());
  }
  ...
  initialize() {
    try {
      ...
      this.logger.debug(
        `Seasonal Training System initialized - Current Phase: ${this.currentPhase}`
      );
      return true;
    } catch (error) {
      ...
      if (workout.exercises && Array.isArray(workout.exercises)) {
        adjustedWorkout.exercises = workout.exercises.map(exercise => {
          ...
          if (adjusted.weight && typeof adjusted.weight === 'number') {
            adjusted.weight = Math.max(
              0,
              Math.round(adjusted.weight * adjustments.intensityMultiplier)
            );
          }
