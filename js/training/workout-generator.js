...
class WorkoutGenerator {
    ...
    return {
      ...
      soccer: [
        ...
        },
        {
          ...
          equipment: 'bodyweight',
          difficulty: 'beginner',
          muscleGroups: ['quadriceps', 'cardio'],
        },
        {
          name: 'Butt Kicks',
          equipment: 'bodyweight',
  ...
  selectExercises(template, focus, sessionType, userProfile) {
    ...
    if (sessionType.includes('Soccer') || userProfile.goals?.secondary === 'athletic_performance') {
      const sportExercise = this.selectBestExercise(
        this.exerciseDatabase.soccer,
        userProfile,
        'sport-specific'
      );
      if (sportExercise) {
  ...
  getExercisesForMuscleGroup(muscleGroup) {
    ...
    for (const category in this.exerciseDatabase.upperBody) {
      if (
        this.exerciseDatabase.upperBody[category].some(ex => ex.muscleGroups.includes(muscleGroup))
      ) {
    ...
    for (const category in this.exerciseDatabase.lowerBody) {
      if (
        this.exerciseDatabase.lowerBody[category].some(ex => ex.muscleGroups.includes(muscleGroup))
      ) {
      ...
      exercises: [
        {
          ...
          duration: 3,
          description: 'Arm circles, leg swings, hip circles',
        },
  ...
  calculateRestPeriods(exercises) {
    try {
      ...
      return exercises.map(exercise => {
        ...
        const restPeriod = Math.round(
          baseRest * intensityMultiplier * weightMultiplier * rpeMultiplier
        );
  ...
  addProgressiveOverload(previousWorkout, currentWorkout) {
    try {
      ...
      const progression = {
        ...
        maxWeightIncrease: 0.1, // Maximum 10% increase per workout
      };
        ...
        if (previousExercise) {
          ...
          if (previousReps) {
            if (typeof currentReps === 'string' && currentReps.includes('-')) {
              ...
              const prevRepsNum =
                typeof previousReps === 'string' && previousReps.includes('-')
  ...
  getExercisesByCategory(category) {
    ...
    return allExercises.filter(
      exercise => exercise.muscleGroups && exercise.muscleGroups.includes(category)
    );
  }
  ...
  selectExerciseForUser(exercises, userProfile, targetMuscleGroup = null) {
    ...
    const suitableExercises = exercises.filter(exercise => {
      ...
      return (
        exercise.difficulty === experienceLevel ||
    ...
    // Use muscle group rotation logic instead of random selection
    return this.selectExerciseWithRotation(suitableExercises, userProfile, targetMuscleGroup);
  }
