...
function resetPassword() {
  ...
  if (!usernameEl || !athleteNameEl || !newPasswordEl || !confirmPasswordEl) {
    return;
  } // Form not available
...
function loadUserData() {
  ...
  if (user.personalData) {
    ...
    if (ageEl && user.personalData.age) {
      ageEl.value = user.personalData.age;
    }

    const weightEl = document.getElementById('weight');
    if (weightEl && user.personalData.weight) {
      weightEl.value = user.personalData.weight;
    }

    const heightEl = document.getElementById('height');
    if (heightEl && user.personalData.height) {
      heightEl.value = user.personalData.height;
    }

    const experienceEl = document.getElementById('experience');
    if (experienceEl && user.personalData.experience) {
      experienceEl.value = user.personalData.experience;
    }
  }
...
function renderOnboardingQuestion() {
  ...
  if (onboardingStep) {
    onboardingStep.textContent = progress.currentStep + 1;
  }

  const onboardingTotal = document.getElementById('onboardingTotal');
  if (onboardingTotal) {
    onboardingTotal.textContent = progress.totalSteps;
  }
...
function updateReadinessSummary() {
  ...
  if (readinessScore && adjustments) {
    ...
    if (readinessSummaryEl) {
      readinessSummaryEl.style.display = 'block';
    }
    if (readinessScoreValueEl) {
      readinessScoreValueEl.textContent = readinessScore;
    }
    if (coachMessageEl) {
      coachMessageEl.textContent = adjustments.coachMessage;
    }
    ...
    if (adjustments.intensityReduced || adjustments.recoverySuggested) {
      ...
      if (workoutAdjustmentsEl) {
        workoutAdjustmentsEl.style.display = 'block';
      }
    ...
    if (completeCheckInBtn) {
      completeCheckInBtn.disabled = false;
    }
  }
}
...
function completeDailyCheckIn() {
  ...
  if (result.success) {
    ...
    proceedWithWorkout();
    showSuccess(
      'Daily check-in completed! Your workout has been adjusted based on your readiness.'
    );
  } else {
    showError(null, result.error);
  }
...
function submitExerciseFeedback() {
  ...
  const exerciseAdapter =
    window.exerciseAdapter || (window.ExerciseAdapter ? new window.ExerciseAdapter() : null);
...
function adaptWorkoutToTime(availableTime) {
  ...
  const adaptedWorkout = window.ProgressionEngine?.adaptWorkoutToTime(
    availableTime,
    plannedWorkout
  );
...
function renderHabits() {
  ...
  const habitProgress = habitTracker.getHabitProgress(
    habitTracker.authManager?.getCurrentUsername()
  );
  const achievements = habitTracker.getUserAchievements(
    habitTracker.authManager?.getCurrentUsername()
  );
  ...
  container.innerHTML = `
        ...
        <div class="achievements-section">
            ...
            <div class="achievements-grid">
                ${habitTracker.achievementDefinitions
                  .map(
                    achievement => `
