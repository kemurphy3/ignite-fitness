...
class ScreeningResults {
  ...
  getUserResults(userId) {
    return this.results
      .filter(result => result.userProfile?.userId === userId)
  ...
  calculateRisk(results) {
    ...
    const asymmetryCount = results.filter(
      r => r.observations && Object.values(r.observations).some(obs => obs.asymmetry)
    ).length;
    ...
    const majorCompCount = results.filter(
      r => r.observations && Object.values(r.observations).some(obs => obs.compensation === 'major')
    ).length;
  ...
  generateRiskRecommendations(riskLevel, factors) {
    ...
    if (riskLevel === 'very_high' || riskLevel === 'high') {
      recommendations.push({
        type: 'reduction',
        message:
          'Significant injury risk detected. Implement comprehensive corrective program immediately.',
        priority: 'critical',
        actions: [
