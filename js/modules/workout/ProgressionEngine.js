...
class ProgressionEngine {
  ...
  async adjustLoadFromRPE(userId, currentWorkout, rpe) {
    try {
      ...
      if (rpe >= 9) {
        ...
        adjustments.coachMessage =
          'Previous session was hard (RPE 8). Slightly reducing load by 3%.';
      } else if (rpe >= 7) {
        // Moderate - maintain
        adjustments.intensityMultiplier = 1.0;
        adjustments.volumeMultiplier = 1.0;
        adjustments.coachMessage =
          'Previous session was moderate (RPE 7). Maintaining current load.';
      } else if (rpe >= 5) {
        // Easy - increase by 5%
        adjustments.intensityMultiplier = 1.05;
        adjustments.volumeMultiplier = 1.05;
        adjustments.coachMessage =
          'Previous session was easy (RPE 5-6). Increasing load by 5% for progressive overload.';
      } else {
        // Very easy - increase by 10%
        adjustments.intensityMultiplier = 1.1;
        adjustments.volumeMultiplier = 1.1;
        adjustments.coachMessage =
          'Previous session was very easy (RPE <5). Increasing load by 10% to challenge adaptation.';
      }
  ...
  getDeloadAdjustments() {
    return {
      ...
      deload: true,
      coachMessage:
        'Deload week detected (every 4th week). Reducing volume by 20% for active recovery and supercompensation.',
    };
  }
