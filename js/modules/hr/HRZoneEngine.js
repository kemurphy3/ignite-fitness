...
class HRZoneEngine {
  ...
  calculateZones(maxHR, restingHR, method = DEFAULT_METHOD, context = {}) {
    ...
    const zones = {
      Z1: {
        min: this.#zoneValue(restingHR, hrReserve, 0.5),
        max: this.#zoneValue(restingHR, hrReserve, 0.6),
      },
      Z2: {
        min: this.#zoneValue(restingHR, hrReserve, 0.6),
        max: this.#zoneValue(restingHR, hrReserve, 0.7),
      },
      Z3: {
        min: this.#zoneValue(restingHR, hrReserve, 0.7),
        max: this.#zoneValue(restingHR, hrReserve, 0.8),
      },
      Z4: {
        min: this.#zoneValue(restingHR, hrReserve, 0.8),
        max: this.#zoneValue(restingHR, hrReserve, 0.9),
      },
      Z5: { min: this.#zoneValue(restingHR, hrReserve, 0.9), max: Math.round(adjustedMax) },
    };
