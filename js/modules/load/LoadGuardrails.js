...
class LoadGuardrails {
    ...
    initializeRecoveryProtocols() {
        return {
            rampExceeded: {
                action: 'reduce_hiit',
                duration: 7, // days
                message: 'Training load increased too quickly. Next high-intensity session will be reduced by {percentage}%.'
            },
            missedDays: {
                action: 'gradual_return',
                rampDown: 0.15, // 15% reduction per missed day
                maxReduction: 0.40, // Maximum 40% reduction
                message: 'Missed training detected. Load reduced by {percentage}% for safe return.'
            },
            painFlag: {
                action: 'immediate_downshift',
                reduction: 0.30, // 30% immediate reduction
                duration: 14, // days
    ...
    analyzeRampRate(rampRate, thresholds, loadHistory) {
        ...
        const consecutiveIncreases = this.checkConsecutiveIncreases(loadHistory, thresholds.maxWeeklyIncrease);
    ...
    async modifyUpcomingHIIT(userId, reduction) {
        ...
        for (const session of hiitSessions.slice(0, 2)) { // Modify next 2 HIIT sessions
