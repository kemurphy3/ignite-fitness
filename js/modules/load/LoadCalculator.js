...
class LoadCalculator {
  ...
  calculateSessionLoad(session = {}) {
    ...
    exercises.forEach(exercise => {
      ...
      if (isSoccerShape) {
        ...
        const tags = Array.isArray(exercise.tags)
          ? exercise.tags.map(tag => String(tag).toLowerCase())
          : [];
        const hasCod = tags.some(
          tag =>
            tag.includes('change_of_direction') || tag.includes('cod') || tag.includes('agility')
        );
        if (hasCod) {
    ...
    const externalActivities = Array.isArray(session.externalActivities)
      ? session.externalActivities
      : [];
    ...
    const intensityRatio =
      totalLoad > 0 ? Math.max(0, Math.min(1, intensityLoad / safeTotalLoad)) : 0;
  ...
  calculateExternalLoad(activities) {
    ...
    safeActivities.forEach(activity => {
      ...
      const dateKey = activity.start_time
        ? this.safeDateKey(activity.start_time)
        : this.safeDateKey();
      if (!dailyLoads[dateKey]) {
   ...
   */
  generateWorkoutIntensityRecommendations(
    currentLoad,
    sevenDayAverage,
    loadSpike = { isSpike: false, ratio: 1.0, severity: 'none' }
  ) {
    const userLevel = this.getUserTrainingLevel();
    const thresholds = this.loadThresholds[userLevel];
  ...
  calculateSoccerShapeLoad(session) {
    ...
    session.structure.forEach(block => {
      if (block.block_type === 'main' && block.intensity) {
        ...
        if (
          block.description?.toLowerCase().includes('shuttle') ||
  ...
  checkLoadProgression(weeklyLoads, experienceLevel = 'intermediate') {
    ...
    const thresholds =
      guardrails.rampRateThresholds[experienceLevel] || guardrails.rampRateThresholds.intermediate;
    ...
    const rampRate = this.calculateRampRate({ total: currentTotal }, { total: previousTotal });
    ...
    return {
      ...
      recommendation:
        rampRate > thresholds.maxWeeklyIncrease
