...
class LoadCalculationEngine {
  ...
  static compute_load(session) {
    ...
    const zoneDistribution =
      session.zone_distribution && Object.keys(session.zone_distribution).length > 0
  ...
  static calculateTRIMP(session) {
    ...
    const max_hr =
      user_profile?.max_hr || this.estimateMaxHR(user_profile?.age, user_profile?.gender);
  ...
  static validateSession(session) {
    ...
    if (session.zone_distribution) {
      const total_zone_minutes = Object.values(session.zone_distribution).reduce(
        (sum, min) => sum + min,
        0
      );
