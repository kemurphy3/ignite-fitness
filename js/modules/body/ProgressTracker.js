class ProgressTracker {
  ...
  #validateHistory(history) {
    ...
    return history.filter(entry => {
      ...
      if (!Number.isFinite(entry.bodyFat) || entry.bodyFat < 2 || entry.bodyFat > 70) {
        return false;
      }
      if (!Number.isFinite(entry.weight) || entry.weight < 70 || entry.weight > 500) {
        return false;
      }
      if (
        entry.muscleMass !== undefined &&
        (!Number.isFinite(entry.muscleMass) || entry.muscleMass < 10)
      ) {
        return false;
      }
      return true;
  ...
  #computeWeeklyChanges(entries) {
    ...
    for (let i = 1; i < entries.length; i++) {
      ...
      const significant =
        Math.abs(changeBF) >= this.significanceThreshold ||
        Math.abs(changeWeight) >= this.significanceThreshold;
      changes.push({
        start: previous.date,
        end: current.date,
