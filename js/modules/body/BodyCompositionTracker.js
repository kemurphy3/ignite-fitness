class BodyCompositionTracker {
  ...
  #calculateTrends(entries) {
    if (entries.length < 2) {
      return { weightChangePerWeek: 0, bodyFatChangePerWeek: 0 };
    }
    const first = entries[0];
    const last = entries[entries.length - 1];
    const weeks = Math.max(
      (new Date(last.date) - new Date(first.date)) / (1000 * 60 * 60 * 24 * 7),
      1
    );
    return {
  ...
  #generateAlerts(entries, trends) {
    ...
    if (recentEntries.length >= 3) {
      const weightVariance =
        Math.max(...recentEntries.map(e => e.weight)) -
        Math.min(...recentEntries.map(e => e.weight));
