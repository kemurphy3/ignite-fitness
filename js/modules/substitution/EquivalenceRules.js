...
export const EquivalenceRules = {
  ...
  loadEquivalenceFactor(fromModality, toModality) {
    ...
    const factors = {
      ...
      running_to_swimming: 1.1,
      swimming_to_running: 1 / 1.1,
      cycling_to_swimming: 0.85 * 1.1,
      swimming_to_cycling: 1 / (0.85 * 1.1),
    };
  ...
  CONFIDENCE_FACTORS: {
    ...
    same_adaptation: 0.1, // +10% if adaptations match exactly
    compatible_adaptation: 0.05, // +5% if adaptations are compatible
    common_conversion: 0.05, // +5% for run<->bike conversions
    zone_penalty: {
      // Confidence reduction by zone
      Z1: 0.0,
      Z2: 0.0,
      Z3: 0.0,
      Z4: -0.05, // -5% for VO2 intervals
      Z5: -0.1, // -10% for power intervals
    },
    duration_penalty: {
      very_short: -0.1, // <10min sessions
  ...
  checkAdaptationCompatibility(sourceAdaptation, targetAdaptation) {
    ...
    const isCompatible = compatibleAdaptations.some(
  ...
  calculateConfidence(params) {
    ...
    if (
      (sourceModality === 'running' && targetModality === 'cycling') ||
