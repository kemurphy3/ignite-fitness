...
class CorrectiveExercises {
  ...
  selectExercises(issues) {
    ...
    issues.forEach(issue => {
      Object.values(this.exerciseLibrary).forEach(exercise => {
        if (
          exercise.purpose.includes(issue) ||
    ...
    return {
      ...
      sessionStructure: {
        mobility: exercises.filter(
          ex =>
            ex.name.toLowerCase().includes('stretch') || ex.name.toLowerCase().includes('mobility')
        ),
        activation: exercises.filter(ex => ex.name.toLowerCase().includes('activation')),
        strength: exercises.filter(
          ex =>
            ex.name.toLowerCase().includes('strength') ||
  ...
  getUserProtocols(userId) {
    return Array.from(this.activeProtocols.values()).filter(protocol => protocol.userId === userId);
  }
  ...
  searchExercises(query) {
    ...
    return Object.values(this.exerciseLibrary).filter(
      exercise =>
    ...
    issuesArray.forEach(issue => {
      ...
      exercises.forEach(exercise => {
        if (
          ...
          exercise.name.toLowerCase().includes('strength') ||
