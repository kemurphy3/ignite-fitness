...
class QuickStart {
  ...
  createCircuit(plan) {
    ...
    return [...mainExercises.slice(0, 2), ...accessories.slice(0, 2)].map(ex => ({
  ...
  getKeyExercises(plan) {
    ...
    // Warmup
    if (plan.warmup) {
      exercises.push(...plan.warmup);
    }

    // Main work
    if (plan.mainSets) {
      exercises.push(...plan.mainSets);
    }

    // Finisher
    if (plan.finishers) {
      exercises.push(...plan.finishers.slice(0, 1));
    }
