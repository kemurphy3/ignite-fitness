...
class DashboardRenderer {
  ...
  generateDashboardHTML(config, role) {
    ...
    if (config.components.lastWorkoutSummary.show) {
      html += this.generateLastWorkoutSection(
        recentWorkouts[0],
        config.components.lastWorkoutSummary
      );
    }
  ...
  getTodaysWorkoutPlan() {
    ...
    return {
      blocks: [
        {
          name: 'Warm-up',
          items: [{ name: 'Dynamic Warm-up', sets: 1, reps: '5-10 min' }],
        },
        {
          name: 'Main Work',
          items: [
  ...
  updateDashboardUI(html) {
    ...
    if (dashboardContainer) {
      ...
      const dashboardContent =
        dashboardContainer.querySelector('.dashboard-content') ||
    ...
    expertCoordinator
      ...
      .then(newPlan => {
        ...
        window.showSuccessNotification?.(
          'Workout plan updated. Normal workout plan generated.',
          'success'
        );
