...
class PersistentHeader {
        ...
        ({ queueLength }) => {
          ...
          if (show) {
            ...
            syncTimer = setTimeout(() => {
              if (right.querySelector('.sync-indicator')) {
                return;
              }
              const el = document.createElement('div');
              el.className = 'sync-indicator';
              el.style.cssText =
                'display:flex;align-items:center;gap:6px;margin-left:8px;color:#9CA3AF;font-size:12px;';
              el.innerHTML =
                '<span class="if-spinner" style="width:14px;height:14px;border-width:2px"></span><span>Syncing…</span>';
    ...
    if (modeBtn) {
      modeBtn.onclick = () => {
        ...
        if (typeof newState === 'boolean') {
          ...
          window.LiveRegionManager?.announce(
            newState ? 'Simple Mode enabled' : 'Advanced Mode enabled',
            'polite'
          );
        }
      };
  ...
  generateHeaderHTML() {
    ...
    const simpleEnabled = authState.isAuthenticated
      ? (window.SimpleModeManager?.isEnabled() ?? true)
      : (window.SimpleModeManager?.isEnabled() ?? true);
  ...
  updateSignInButton() {
    ...
    if (authState.isAuthenticated && existingBtn) {
      ...
      signInBtn.style.cssText =
        'padding: 0.5rem 1rem; background: #4299e1; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.875rem; font-weight: 600;';
