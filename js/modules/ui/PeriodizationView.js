...
class PeriodizationView {
    ...
    toggleCalendar() {
        ...
        // Focus management
        const focusable = modal.querySelectorAll('[data-close], [data-prev], [data-next], [data-today], .if-cal-cell');
        ...
        const rerender = () => { bodyEl.innerHTML = this.renderCalendarGrid(current); this.wireCalendarCells(modal, current); };

        // Wire controls
        modal.querySelectorAll('[data-close]').forEach(btn => btn.addEventListener('click', () => modal.remove()));
        modal.querySelector('[data-prev]').addEventListener('click', () => { current = new Date(current.getFullYear(), current.getMonth() - 1, 1); rerender(); });
        modal.querySelector('[data-next]').addEventListener('click', () => { current = new Date(current.getFullYear(), current.getMonth() + 1, 1); rerender(); });
        modal.querySelector('[data-today]').addEventListener('click', () => { current = new Date(); rerender(); });
