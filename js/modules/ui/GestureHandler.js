...
class GestureHandler {
  ...
  handleSwipe(gestureData, direction) {
    ...
    // Emit custom event
    gestureData.element.dispatchEvent(
  ...
  handleDoubleTap(gestureData) {
    ...
    // Emit custom event
    gestureData.element.dispatchEvent(
      new CustomEvent('gesture:doubletap', {
        detail: { target, action },
      })
    );
  ...
  getGestureTarget(element) {
    ...
    if (element.classList.contains('exercise-card')) {
      return 'exercise_card';
    }
    if (element.classList.contains('workout-item')) {
      return 'workout_item';
    }
    if (element.classList.contains('progress-chart')) {
