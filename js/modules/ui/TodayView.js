...
class TodayView {
    ...
    async loadPlannedSession() {
        try {
            ...
            // Try API
            const response = await fetch(`/.netlify/functions/sessions-planned?date=${this.todayData.date}`, {
                headers: {
                    'Authorization': `Bearer ${this.authManager?.getToken() || ''}`
                }
            });
    ...
    async loadCompletedSessions() {
        try {
            ...
            const todaySessions = localSessions.filter(session =>
                session.date === this.todayData.date
            );
            ...
            // Try API
            const response = await fetch(`/.netlify/functions/sessions-list?date=${this.todayData.date}`, {
                headers: {
                    'Authorization': `Bearer ${this.authManager?.getToken() || ''}`
                }
            });
