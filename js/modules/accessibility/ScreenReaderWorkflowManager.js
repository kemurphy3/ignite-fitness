...
class ScreenReaderWorkflowManager {
  ...
  handleKeyboardShortcuts(e) {
    if (!this.userPreferences.shortcutKeys) {
      return;
    }

    const shortcut = Array.from(this.shortcuts.values()).find(
      s => s.key === e.key.toLowerCase() && s.ctrl === e.ctrlKey
    );
  ...
  createSimplifiedWorkoutInterface() {
    ...
    // Add event listeners
    document
      .getElementById('sr-next-exercise')
      .addEventListener('click', () => this.nextExercise());
    document
      .getElementById('sr-previous-exercise')
      .addEventListener('click', () => this.previousExercise());
  ...
  generateHelpContent() {
    ...
    content +=
      '<p>Audio cues are played for important workout events. You can disable them in preferences.</p>';

    content += '<h3>Simplified Mode</h3>';
    content +=
      '<p>Simplified mode reduces visual clutter and focuses on essential information.</p>';
  ...
  playAudioCue(cueName) {
    if (!this.userPreferences.audioCues) {
      return;
    }
