...
class IntegrationPanel extends BaseComponent {
  ...
  async loadIntegrations() {
    try {
      ...
      if (response.ok) {
        ...
        this.integrations = new Map(
          data.integrations.map(integration => [integration.provider, integration])
        );
      }
    } catch (error) {
  ...
  async connectStrava() {
    ...
    if (!stravaAuthUrl) {
      ...
      this.showError(
        'Strava configuration required. Please set STRAVA_CLIENT_ID in your environment.'
      );
      return;
    }
  ...
  getStravaIntegrationConfig() {
    try {
      const integrations =
        typeof window !== 'undefined' && window.configLoader?.get?.('integrations')
          ? window.configLoader.get('integrations')
          : {};
      const strava = integrations?.strava || {};
      const fallbackRedirect =
        typeof window !== 'undefined' ? `${window.location.origin}/auth/strava/callback` : '';
      return {
