...
class ConsentManager extends BaseComponent {
  ...
  checkCompliance() {
    ...
    const requiredConsent = this.consentTypes.filter(type => this.isConsentRequired(type));
  ...
  async acceptAllConsent() {
    try {
      const consentTypes = this.consentTypes.filter(type => this.isConsentRequired(type));
  ...
  showConsentBannerIfNeeded() {
    const requiredConsent = this.consentTypes.filter(
      type => this.isConsentRequired(type) && !this.hasConsent(type)
    );
