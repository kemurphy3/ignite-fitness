    ...
    try {
      ...
      if (response.ok) {
        ...
        await this.logAuditEvent('data_export', {
          ...
          method: 'server-side',
          size:
            typeof data.export_data === 'string'
  ...
  async loadDataStats() {
    try {
      ...
      if (response.ok) {
        ...
        if (workoutCount) {
          workoutCount.textContent = data.workout_count || 0;
        }
        if (activityCount) {
          activityCount.textContent = data.activity_count || 0;
        }
        if (oldestData) {
          oldestData.textContent = data.oldest_data || 'N/A';
        }
      }
    } catch (error) {
