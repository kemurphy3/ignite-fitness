...
class CoachChat {
  ...
  openChat() {
    ...
    if (this.conversationContext.length > 0) {
      this.addMessage(
        'coach',
        'Welcome back! I remember our earlier conversation. How can I help you today?'
      );
    } else {
  ...
  async sendMessage() {
    ...
    const typingTimer = setTimeout(() => {
      ...
      typingDiv.innerHTML =
        '<div class="message-bubble"><span class="if-spinner" style="vertical-align: middle; margin-right: 8px;"></span>Coach is thinking…</div>';
      ...
      if (isFollowUp && context.length > 0) {
        ...
        if (lastExchange.coach && lastExchange.coach.includes('intensity')) {
          return {
            ...
            modifications: { intensityMultiplier: 0.6 }, // Further reduction
          };
        }
      }
  ...
  detectFollowUp(message, context) {
    ...
    const hasPronoun = pronouns.some(
      pronoun =>
   ...
   */
  getQuickSuggestions() {
    return ['Too hard', 'Less time', 'Equipment missing', 'Something hurts', 'Different workout'];
  }
