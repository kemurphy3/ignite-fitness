...
class PersonalizedCoaching {
  ...
  initializeBehavioralTriggers() {
    return {
      ...
      celebrationMessages: {
        ...
        streak_14:
          '🔥 2 weeks of consistency! Your body is adapting to the routine. This is where change happens!',
        streak_30:
          "💪 A full month! You're in the top 10% of people who start a fitness journey. You've made it a lifestyle!",
        streak_60:
          "🏆 2 months straight! You've proven you can do this long-term. Your future self will thank you!",
        streak_90:
          "⭐ 90 days! You've built a non-negotiable habit. This is who you are now - someone who takes care of themselves!",
        breakthrough:
          "🚀 Breakthrough detected! You just crushed a plateau. That's the power of consistency paying off!",
        consistency_high:
          "✨ Your consistency is impressive - {percentage}% of your planned workouts completed. That's the secret to results!",
        comeback:
          "💚 Welcome back! Taking breaks is part of the journey. You're here now, that's what matters!",
      },
      plateauStrategies: [
        {
          ...
          strategy: 'deload',
          message:
            'Time for a deload week! Drop to 70% volume but keep the movement quality high. This will supercharge your gains when you come back strong.',
        },
        {
          trigger: 'weight_plateau',
          strategy: 'variation',
          message:
            "Let's try a variation! Switch {exercise} to {variation} - sometimes a new angle is all your muscles need to grow.",
        },
        {
          trigger: 'time_plateau',
          strategy: 'frequency',
          message:
            "Instead of longer sessions, let's try more frequent shorter workouts. Sometimes volume distributed differently breaks through plateaus.",
        },
      ],
    };
  }
  ...
  initializeCoachingTemplates() {
    return {
      progression: {
        ...
        regression:
          "I noticed your {exercise} weight dropped a bit. No worries - this happens when life gets busy or stress is high. Let's focus on getting back to your baseline first.",
      },
      motivation: {
        streak:
          'Week {number} in the books. You are building a real habit here. Consistency like this is what separates people who see results from those who do not.',
        return:
          "Welcome back! Taking {days} days off wasn't a setback - sometimes your body needs that reset. Let's ease back in with a lighter session.",
        consistency:
          "You've been crushing it with {frequency} workouts per week! This kind of consistency is where the magic happens.",
        comeback:
          'I love seeing you back in the gym! That break actually might have done you some good - fresh start, renewed motivation.',
      },
      performance: {
        high_rpe:
          "You've been pushing hard lately (average RPE of {rpe}+). Your dedication is awesome, but let's add an extra recovery day this week to avoid burnout.",
        low_energy:
          "I noticed your energy levels have been lower. Let's focus on sleep quality and maybe try some lighter, more enjoyable workouts.",
        plateau:
          'Your progress has leveled off - this is totally normal! It means your body has adapted. Time to switch things up with some new challenges.',
        breakthrough:
          "Your recent workouts show you're getting stronger! That extra effort is translating into real gains.",
      },
      recovery: {
        soreness:
          "High soreness detected - let's focus on mobility and recovery today. Your muscles are telling you they need some TLC.",
        stress:
          "I see you're dealing with some stress. Exercise is great for that, but let's keep the intensity moderate - we want to help, not add to the stress.",
        fatigue:
          "Your energy levels suggest you might be overreaching. Let's dial it back and focus on quality over quantity today.",
        readiness:
          "Your readiness score is {score}/10. Based on how you're feeling, let's adjust today's workout to match your current state.",
      },
      seasonal: {
        preseason:
          "Since {sport} season starts in {weeks} weeks, we're focusing on explosive power and agility. Today's plyometrics will help your first step quickness - think of it as training your muscles to fire faster when you need to beat a defender.",
        in_season:
          "You're in the middle of {sport} season, so we're maintaining your strength while keeping you fresh for games. Today's session will support your performance without wearing you out.",
        offseason:
          'Perfect time to build that strength base! With no games to worry about, we can focus on getting you stronger for next season.',
        transition:
          "Great time to try something new! Let's explore some different training methods while you're between seasons.",
      },
      nutrition: {
        energy:
          'Your energy levels suggest we should look at your nutrition timing. Try having a small snack 30-60 minutes before your workout - it can make a huge difference.',
        recovery:
          "Your recovery seems slower than usual. Make sure you're getting enough protein and staying hydrated - your muscles need fuel to rebuild.",
        performance:
          "Your performance has been solid, which suggests your nutrition is on point. Keep doing what you're doing!",
      },
      injury: {
        concern:
          "I'm not a doctor, but if you're experiencing pain during {exercise}, let's skip it today and try {alternative}. When in doubt, it's always better to be safe.",
        prevention:
          "Let's focus on proper form and controlled movements today. Good technique is your best injury prevention.",
        recovery:
          "Take it easy and listen to your body. If something doesn't feel right, we can always modify or skip it.",
      },
    };
  }
  ...
  generateResponse(userMessage, context = null) {
    try {
      ...
      return {
        success: false,
        response:
          "I'm having trouble processing that right now. Can you try rephrasing your question?",
  ...
  detectIntent(message) {
    ...
    if (message.includes('how') || message.includes('why')) {
      return 'explanation';
    }
    if (message.includes('hurt') || message.includes('pain')) {
      return 'concern';
    }
  ...
  detectUrgency(message) {
    ...
    if (message.includes('asap') || message.includes('quickly')) {
      return 'medium';
    }
    return 'low';
  }
  ...
  determineCoachingScenario(context, messageAnalysis) {
    ...
    if (
      messageAnalysis.intent === 'fatigue' ||
    ...
    if (
      messageAnalysis.intent === 'plateau' ||
    ...
    if (
      messageAnalysis.keywords.includes('season') ||
    ...
    if (
      messageAnalysis.keywords.includes('hard') ||
    ...
    if (
      messageAnalysis.intent === 'motivation' ||
  ...
  createRecoveryResponse(context) {
    ...
    return this.coachingTemplates.recovery.readiness.replace('{score}', context.readinessScore);
  }
  ...
  createSeasonalResponse(context) {
    ...
    if (phase === 'in_season') {
      return this.coachingTemplates.seasonal.in_season.replace('{sport}', sport);
    }
  ...
  createPerformanceResponse(context) {
    if (context.averageRPE > 8.5) {
      return this.coachingTemplates.performance.high_rpe.replace(
        '{rpe}',
        context.averageRPE.toFixed(1)
      );
    }
  ...
  createMotivationResponse(context) {
    if (context.workoutStreak > 7) {
      return this.coachingTemplates.motivation.streak.replace(
        '{number}',
        Math.floor(context.workoutStreak / 7)
      );
    }

    return this.coachingTemplates.motivation.consistency.replace(
      '{frequency}',
  ...
  async handleWorkoutCompletion(data) {
    try {
      ...
      const milestone = this.behavioralTriggers.streakMilestones.find(
        m => streak >= m && this.lastTriggeredStreak < m
      );
      ...
      if (
        consistencyScore >= this.behavioralTriggers.consistencyThreshold &&
  ...
  triggerStreakCelebration(milestone, currentStreak) {
    ...
    const message =
      this.behavioralTriggers.celebrationMessages[messageKey] ||
  ...
  triggerConsistencyCelebration(consistencyScore) {
    ...
    const message = this.behavioralTriggers.celebrationMessages.consistency_high.replace(
      '{percentage}',
      percentage
    );
  ...
  detectPlateauAndSuggestStrategy(context) {
    ...
    if (context.progressionRate < 0.01 && context.workoutStreak > 14) {
      const strategy = this.behavioralTriggers.plateauStrategies.find(
        s => s.trigger === 'volume_plateau'
      );
      if (strategy) {
        this.lastPlateauDetection = {
          type: 'volume',
    ...
    if (context.progressionRate < 0.02 && context.workoutStreak > 21) {
      ...
      if (strategy) {
        ...
        return {
          type: 'plateau',
          strategy: strategy.strategy,
          message: strategy.message.replace(
            '{exercise}',
            context.primaryExercise || 'this exercise'
          ),
        };
      }
    }
  ...
  applyPersonalityTraits(response, context) {
    ...
    if (personalizedResponse.includes('deload')) {
      personalizedResponse +=
        ' A deload week gives your nervous system a chance to recover while maintaining your movement patterns.';
    }
