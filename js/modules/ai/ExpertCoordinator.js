...
class ExpertCoordinator {
  constructor() {
    ...
    this.memoizedCoordinator = this.instantiateOrFallback(window.MemoizedCoordinator, () =>
    ...
    this.experts = {
      strength: this.instantiateOrFallback(window.StrengthCoach, () =>
        this.createDefaultExpert('strength')
      ),
      sports: this.instantiateOrFallback(window.SportsCoach, () =>
        this.createDefaultExpert('sports')
      ),
      physio: this.instantiateOrFallback(window.PhysioCoach, () =>
        this.createDefaultExpert('physio')
      ),
      nutrition: this.instantiateOrFallback(window.NutritionCoach, () =>
        this.createDefaultExpert('nutrition')
      ),
      aesthetics: this.instantiateOrFallback(window.AestheticsCoach, () =>
        this.createDefaultExpert('aesthetics')
      ),
      climbing: this.instantiateOrFallback(window.ClimbingCoach, () =>
        this.createDefaultExpert('climbing')
      ),
    };
  ...
  getSessionPlan(context) {
    try {
      ...
      if (
        proposals._empty ||
        Object.values(proposals).every(p => !p || !p.blocks || p.blocks.length === 0)
      ) {
  ...
  mergeProposals(proposals, context) {
    ...
    if (proposals.sports?.blocks) {
      ...
      const gameDayConstraints = proposals.sports?.constraints?.find(
    ...
    if (context.user?.sport === 'climbing' && proposals.climbing?.blocks) {
      ...
      const fingerRecovery = proposals.climbing?.constraints?.find(
        c => c.type === 'finger_recovery'
      );
      if (fingerRecovery) {
        plan.notes.push({
          source: 'climbing',
  ...
  resolveConflicts(plan, proposals, context) {
    ...
    const kneePain =
      proposals.physio?.blocks?.find(b => b.exercise?.rationale?.toLowerCase().includes('knee')) ||
  ...
  generateSessionNotes(plan, context) {
    ...
    if (plan.accessories.length > 0) {
      notes.push(
        `Accessories: ${context.preferences?.aestheticFocus} focus (${plan.accessories.length} exercises)`
      );
    }
  ...
  applyLoadBasedAdjustments(context) {
    ...
    if (yesterday.z4_min >= 20 || yesterday.z5_min >= 10) {
      ...
      context.loadAdjustments.push(
        `Synced HR shows ${yesterday.z4_min} min in Z4 and ${yesterday.z5_min} min in Z5 yesterday → dialing back lower-body volume.`
      );
      this.logger.info('Suppressing heavy lower due to high-intensity yesterday', {
        z4_min: yesterday.z4_min,
        z5_min: yesterday.z5_min,
      });
    }
    ...
    if (load.strain > 150 || (load.monotony > 2.0 && load.atl7 > load.ctl28 * 1.2)) {
      ...
      context.loadAdjustments.push(
        `High weekly strain detected (${load.strain}) or monotony > 2.0 → adding mobility emphasis.`
      );
      this.logger.info('Recommending deload due to high strain/monotony', {
        strain: load.strain,
    ...
    if (readinessProxy < 0.8) {
      ...
      context.loadAdjustments.push(
        `Rolling load suggests lower readiness → scaling intensity to ${(readinessProxy * 100).toFixed(0)}%.`
      );
      this.logger.info('Scaling intensity based on readiness proxy', {
        proxy: readinessProxy,
        scaledIntensity: context.intensityScale,
      });
    }
    ...
    if (dataConfidence.recent7days < 0.5) {
      ...
      context.loadAdjustments.push(
        `Limited HR data this week (confidence ${(dataConfidence.recent7days * 100).toFixed(0)}%) → conservative recommendation.`
      );
      this.logger.info('Using conservative mode due to low data confidence', {
        confidence: dataConfidence.recent7days,
      });
    }
  }
  ...
  applyLoadBasedWorkoutAdjustments(context, loadStatus) {
    ...
    if (recommendations.intensity < 1.0) {
      ...
      context.loadAdjustments.push(
        `Load-based intensity reduction: ${recommendations.adjustments.intensityReduction}%`
      );
    }
    ...
    if (recommendations.volume < 1.0) {
      ...
      context.loadAdjustments.push(
        `Load-based volume reduction: ${recommendations.adjustments.volumeReduction}%`
      );
    }
  ...
  normalizeReadiness(context) {
    const readinessValue = Number(context.readiness ?? context.readinessScore ?? 7);
    const readiness = Math.max(
      1,
      Math.min(10, Number.isFinite(readinessValue) ? readinessValue : 7)
    );
  ...
  buildBasePlan(context, readiness) {
    ...
    return {
      ...
      notes: [],
      why: ['Base plan assembled from core expert templates', `Readiness input: ${readiness}/10`],
      metadata: {
        source: 'base',
      },
    };
  ...
  applyReadinessAdjustments(plan, readiness) {
    ...
    if (multiplier < 1) {
      ...
      plan.why.push(
        `Readiness ${readiness}/10 → intensity increased by ${(multiplier - 1) * 100}%`
      );
    } else {
  ...
  applyConservativeDefaults(context) {
    ...
    if (
      ...
      isNaN(safeContext.readiness) ||
      safeContext.readiness < 1 ||
      safeContext.readiness > 10
    ) {
  ...
  applyHeartRateInfluence(plan, context) {
    ...
    if (hrData.zoneDistribution) {
      const highIntensityMinutes =
        (hrData.zoneDistribution.Z4 || 0) + (hrData.zoneDistribution.Z5 || 0);
      ...
      if (plannedHighIntensity && highIntensityMinutes > plannedHighIntensity) {
        plan.notes.push(
          'Recent training skewed toward high intensity – scheduling deload elements.'
        );
  ...
  applyPersonalAIAdjustments(plan, context) {
    ...
    try {
      ...
      const mainCandidates = (plan.mainSets || [])
        .map((item, index) => ({
      ...
      if (mainCandidates.length > 0) {
        ...
        if (recommendation?.choice) {
          ...
          plan.metadata.combinedConfidence = Number(
            (
              (plan.metadata.baseConfidence || 0.6) * 0.7 +
              recommendation.metadata.combinedConfidence * 0.3
            ).toFixed(2)
          );
          plan.notes.push(
            `Personal AI weighting applied (${Math.round(plan.metadata.personalAI.combinedConfidence * 100)}% confidence).`
          );
          plan.why.push('Prioritized exercises that historically perform well for you.');

          // Bubble preferred exercise to top if different
          const preferredIndex = plan.mainSets.findIndex(
            item => (item.exercise || item.name) === recommendation.choice.name
          );
          if (preferredIndex > 0) {
