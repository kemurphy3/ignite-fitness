...
class AdaptiveRecommender {
    ...
    recommend(payload = {}) {
        ...
        if (!userId || !candidates.length) {
            ...
            return { choice: candidates[0] || null, metadata: { confidence: baseConfidence, strategy: 'fallback' } };
        }
        ...
        const preferenceMap = new Map(preferredExercises.map(item => [item.name, item.preferenceScore]));

        const scored = candidates.map(candidate => {
            const prior = this._derivePrior(candidate);
            const personalConfidence = preferenceMap.get(candidate.name) || personalPatterns.exercises?.[candidate.name]?.preferenceScore || 0.5;
