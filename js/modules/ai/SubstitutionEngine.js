...
class SubstitutionEngine {
    ...
    try {
      ...
      return substitutions
        .sort((a, b) => {
          ...
          return (
            this.scorePreferenceMatch(b, constraints) - this.scorePreferenceMatch(a, constraints)
          );
        })
        .slice(0, 3);
    } catch (error) {
  ...
  async findEquivalentWorkout(
    ...
    try {
      ...
      const candidateWorkouts = allWorkouts.filter(workout => {
        ...
        return (
          workoutAdaptation.toLowerCase().includes(adaptation.toLowerCase()) ||
      ...
      for (const candidate of candidateWorkouts) {
        ...
        if (variance < smallestVariance && variance <= 0.15) {
          // Within 15% load variance
        ...
        if (block.type === 'main') {
          ...
          if (block.sets) {
            ...
            const zoneAdjustment =
              this.modalityFactors.zoneAdjustments[workIntensity]?.[targetModality] || 1;
  ...
  calculateWorkoutLoad(workout) {
    ...
    for (const block of workout.structure) {
      ...
      const multiplier =
        zoneMultipliers[intensity] || zoneMultipliers[intensity.split('-')[0]] || 1;
  ...
  calculateLoadVariance(originalLoad, substitutedLoad) {
    if (!originalLoad || originalLoad === 0) {
      return 1;
    }
  ...
  validateSubstitution(substitution, constraints = {}) {
    ...
    if (substitution.equipment && substitution.equipment.length > 0) {
      if (!this.hasRequiredEquipment(substitution.equipment, constraints)) {
        this.logger.debug(
          `Substitution requires unavailable equipment: ${substitution.equipment.join(', ')}`
        );
        return false;
      }
    }
    ...
    const timeRequired =
      substitution.timeRequired || this.calculateTotalTime(substitution.structure || []);
    ...
    if (todayLoad + substitutionLoad > this.guardrails.dailyLoadCap) {
      this.logger.debug(
        `Substitution exceeds daily load cap: ${todayLoad + substitutionLoad} > ${this.guardrails.dailyLoadCap}`
      );
      return false;
    }
  ...
  detectModality(workout) {
    if (workout.modality) {
      return workout.modality;
    }
    if (workout.equipment && workout.equipment.includes('pool')) {
      return 'swimming';
    }
    if (workout.equipment && workout.equipment.includes('track')) {
      return 'running';
    }
    if (workout.equipment && workout.equipment.includes('bike')) {
      return 'cycling';
    }
    ...
    if (name.includes('bike') || name.includes('cycle') || name.includes('ride')) {
      return 'cycling';
    }
    if (name.includes('run') || name.includes('track') || name.includes('mile')) {
      return 'running';
    }
  ...
  calculateWeeklyHardMinutes(recentSessions = []) {
    ...
    return recentSessions
      ...
      .reduce((total, session) => {
        ...
        const hardMinutes =
          session.hardMinutes || this.calculateHardMinutes(session.workout || session);
