...
class DebugManager {
  ...
  debugPerformance() {
    ...
    if (performance.memory) {
      ...
      this.logger.log(
        `Memory total: ${(performance.memory.totalJSHeapSize / 1024 / 1024).toFixed(2)} MB`
      );
      this.logger.log(
        `Memory limit: ${(performance.memory.jsHeapSizeLimit / 1024 / 1024).toFixed(2)} MB`
      );
    }
  ...
  clearDebugData() {
    ...
    const debugKeys = Object.keys(localStorage).filter(
      key => key.startsWith('ignitefitness_debug_') || key.startsWith('ignitefitness_test_')
    );
