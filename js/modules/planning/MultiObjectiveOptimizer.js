...
class MultiObjectiveOptimizer {
  ...
  optimizeTrainingPlan(goals, constraints, timeHorizon) {
    ...
    let population = this.generateInitialSolutions(
      feasibleConstraints,
      this.populationSize,
      timeHorizon
    );
  ...
  #crossover(parentA, parentB, timeHorizon) {
    ...
    const point = Math.floor(
      this.random() * Math.min(parentA.schedule.length, parentB.schedule.length)
    );
    ...
    mutated.schedule = mutated.schedule.map(session => {
      ...
      if (this.random() < this.mutationRate) {
        ...
        updated.duration = Math.min(
          constraints.maxDailyDuration,
          Math.max(30, updated.duration * factor)
        );
      }
      return updated;
    });
  ...
  #computeFatigue(schedule, timeHorizon = schedule.length) {
    const total = schedule.reduce(
      (sum, session) => sum + (session.duration * session.intensity) / timeHorizon,
      0
    );
    return total / 90;
  }
