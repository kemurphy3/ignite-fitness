...
class PeriodizationEngine {
  ...
  designPeriodizationPlan(goals, constraints = {}, calendar = {}) {
    ...
    const phases = phaseTemplate.map(phase => {
      const weeklyPlan = this.#constructWeeklyPlan(
        phase,
        sessionsPerWeek,
        goals,
  ...
  #buildPhaseTemplate(timeFrame, goals) {
    ...
    const blockLengths = [
      ...
      Math.round(timeFrame * 0.35),
      Math.round(timeFrame * 0.2),
    ];
    const maintenanceWeeks = timeFrame - (blockLengths[0] + blockLengths[1] + blockLengths[2]);
    if (maintenanceWeeks > 0) {
      blockLengths[2] += maintenanceWeeks;
    }
    return [
      {
        name: 'Accumulation',
        weeks: blockLengths[0],
        emphasis: emphasisMap.accumulation,
        intensity: 'moderate',
        focus: 'volume',
      },
      {
        name: 'Intensification',
        weeks: blockLengths[1],
        emphasis: emphasisMap.intensification,
        intensity: 'high',
        focus: 'intensity',
      },
      {
        name: 'Realization',
        weeks: blockLengths[2],
        emphasis: emphasisMap.realization,
        intensity: 'peak',
        focus: 'performance',
      },
    ];
  ...
  #estimatePerformanceGain(phases, constraints) {
    ...
    const volumes = phases.map(
      phase => phase.weeklyPlan.reduce((sum, session) => sum + session.sessionLoad, 0) * phase.weeks
    );
    const intensities = phases.map(
      phase =>
        phase.weeklyPlan.reduce((sum, session) => sum + session.intensity, 0) /
        phase.weeklyPlan.length
    );
    const volumeScore =
      volumes.reduce((sum, value, index) => sum + value * (index + 1), 0) / (volumes.length || 1);
    const intensityScore =
      intensities.reduce((sum, value) => sum + value, 0) / (intensities.length || 1);
    const fatiguePenalty =
      (fatigueSensitivity * volumes.reduce((sum, value) => sum + value, 0)) / 1500;
