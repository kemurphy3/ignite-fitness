...
class InterferenceEngine {
    ...
    calculateInterference(sessionPlan, recoveryProfile = {}) {
        ...
        const aggregated = outputs.reduce((acc, current) => {
        ...
        average.total = Math.min(1, average.molecular + average.hormonal + average.glycogen + average.recovery);
    ...
    #molecularSignaling(session, intervalHours) {
        ...
        if (!hasStrength || !hasEndurance) {return 0;}
    ...
    #glycogenAvailability(session, previous, recoveryProfile) {
        const enduranceHeavy = session.modalities.includes('endurance') || session.modalities.includes('conditioning');
        if (!enduranceHeavy) {return 0;}
        const previousGlycogenUse = previous
            ? (previous.duration * previous.intensity * (previous.modalities.includes('endurance') ? 1.2 : 0.6))
            : 0;
    ...
    #recoveryCompetition(session, intervalHours, recoveryProfile) {
        if (!Number.isFinite(intervalHours)) {intervalHours = Infinity;}
        const recoveryRate = recoveryProfile.recoveryRate ?? 0.85;
        const recovered = 1 - Math.pow(this.timeDecay * (1 - recoveryRate), Math.max(intervalHours, 0) / 12);
