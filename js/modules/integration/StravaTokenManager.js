...
class StravaTokenManager {
  constructor(options = {}) {
    ...
    this.authManager =
      options.authManager || (typeof window !== 'undefined' ? window.AuthManager : null);
  ...
  async getTokenStatus(userId) {
    ...
    const response = await this._authorizedFetch(
      `${this.baseUrl}/.netlify/functions/strava-token-status?userId=${encodeURIComponent(userId)}`
    );
  ...
  async refreshToken(userId) {
    const response = await this._authorizedFetch(
      `${this.baseUrl}/.netlify/functions/strava-refresh-token`,
      {
        method: 'POST',
        headers: {
