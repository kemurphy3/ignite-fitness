...
const StravaTokenManagerClass =
  typeof window !== 'undefined' && window.StravaTokenManager
...
class StravaHook {
  constructor(options = {}) {
    ...
    this.storageManager =
      options.storageManager || (typeof window !== 'undefined' ? window.StorageManager : null);
    this.tokenManager =
      options.tokenManager ||
      (StravaTokenManagerClass ? new StravaTokenManagerClass({ logger: this.logger }) : null);
    this.activityMatcher =
      options.activityMatcher ||
      (ActivityMatcherClass ? new ActivityMatcherClass({ logger: this.logger }) : null);
    this.stravaProcessor =
      options.stravaProcessor || (typeof window !== 'undefined' ? window.StravaProcessor : null);
  ...
  async syncActivities(userId, options = {}) {
    ...
    try {
      ...
      const activitiesWithStreams = await this.attachStreams(
      ...
      const matches = this.activityMatcher
        ? this.activityMatcher.matchActivities(activitiesWithStreams, sessions)
        : [];
    ...
    for (const activity of activities) {
      try {
        ...
        if (error instanceof UnauthorizedError && this.tokenManager && !activity.__retried) {
          ...
          const streams = await this.fetchActivityStreams(
          ...
          this.logger.warn?.('StravaHook: failed to fetch streams', {
