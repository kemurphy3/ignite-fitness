const ActivityMatcherModule = (typeof window !== 'undefined' && window.ActivityMatcher)
...
class StravaDataProcessor {
    ...
    async saveActivity(activity) {
        try {
            ...
            if (this.hasExternalActivity(userId, activity.externalId)) {
                this.logger.info('Activity already persisted, skipping storage', { externalId: activity.externalId });
                return;
            }
    ...
    hasExternalActivity(userId, externalId) {
        ...
        const activities = this.getExternalActivities(userId) || [];
        return activities.some(activity => ActivityMatcherModule?.isDuplicate(activity, { id: externalId }));
    }
}
