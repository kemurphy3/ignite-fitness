const ActivityMatcherModule =
  typeof window !== 'undefined' && window.ActivityMatcher
...
class StravaDataProcessor {
  ...
  processActivity(activity) {
    try {
      ...
      const processedActivity = {
        ...
        heartRateSeries: Array.isArray(activity.heartRateSeries) ? activity.heartRateSeries : [],
        ...
        source: 'strava',
        externalId: activity.id ? activity.id.toString() : sanitizedActivity.external_id,
        rawData: activity, // Store original for audit
      };
  ...
  estimatePerceivedExertion(activity) {
    ...
    if (duration > 2) {
      return 9;
    } // Long duration = high exertion
    if (distance > 20) {
      return 8;
    } // Long distance = high exertion
    if (duration > 1) {
      return 7;
    } // Medium duration = medium exertion
    if (distance > 10) {
      return 6;
    } // Medium distance = medium exertion
  ...
  async saveActivity(activity) {
    try {
      ...
      if (this.hasExternalActivity(userId, activity.externalId)) {
        this.logger.info('Activity already persisted, skipping storage', {
      ...
      const activityData = {
        ...
        heart_rate_series: activity.heartRateSeries || [],
  ...
  getActivitySummary() {
    try {
      ...
      const summary = {
        ...
        averageTSS:
          activities.length > 0
            ? activities.reduce((sum, activity) => sum + (activity.training_stress_score || 0), 0) /
              activities.length
            : 0,
        totalRecoveryDebt: activities.reduce(
          (sum, activity) => sum + (activity.recovery_debt_hours || 0),
          0
        ),
  ...
  hasExternalActivity(userId, externalId) {
    if (!externalId) {
      return false;
    }
    const activities = this.getExternalActivities(userId) || [];
    return activities.some(activity =>
      ActivityMatcherModule?.isDuplicate(activity, { id: externalId })
    );
  }
}
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  module.exports.default = StravaDataProcessor;
}
