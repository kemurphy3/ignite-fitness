...
class PredictionEngine {
    constructor(options = {}) {
        ...
        this.seasonLength = Number.isFinite(options.seasonLength) ? Math.max(3, options.seasonLength) : 7;
    ...
    computeDirectionalAccuracy(actualSeries, forecastSeries) {
        ...
        if (!Array.isArray(forecastSeries) || forecastSeries.length === 0) {return 0;}
    ...
    backtestPerformance(series, horizon = 7) {
        ...
        const meanAccuracy = accuracyScores.reduce((sum, score) => sum + score, 0) / (accuracyScores.length || 1);
    ...
    calculateMAPE(actualSeries, forecastSeries) {
        const actual = this.preprocessSeries(actualSeries);
        if (!Array.isArray(forecastSeries) || forecastSeries.length === 0) {return Infinity;}
    ...
    #residualVariance(residuals) {
        ...
        return residuals.reduce((sum, r) => sum + Math.pow(r - meanResidual, 2), 0) / Math.max(residuals.length - 1, 1);
    }
