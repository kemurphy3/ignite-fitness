...
class ModelValidator {
  constructor(options = {}) {
    ...
    this.predictionEngine =
      options.predictionEngine || new PredictionEngine(options.predictionOptions || {});
  ...
  backtest(series, horizon = 7) {
    ...
    folds.forEach(({ training, testing }) => {
      ...
      const accuracy = this.predictionEngine.computeDirectionalAccuracy(
        [...training, ...testing],
        forecasts
      );
  ...
  detectDrift(history, recent, metricKey) {
    ...
    const lastHistory =
      historyFeature.at(-1)[`${metricKey}_ma_${this.featureExtractor.windows[0]}`];
