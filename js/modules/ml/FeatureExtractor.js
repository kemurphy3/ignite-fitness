...
class FeatureExtractor {
    ...
    validateSeries(series = [], requiredKeys = []) {
        ...
        return series
            .map(entry => {
                ...
                normalized.timestamp = Number(entry.timestamp ?? (entry.date ? new Date(entry.date).getTime() : NaN));
    ...
    addRateOfChange(series, metricKeys = []) {
        ...
        for (let i = 1; i < sorted.length; i++) {
            ...
            metricKeys.forEach(key => {
                ...
                features[i][accelKey] = deltaTime > 0 ? (features[i][rateKey] - previousRate) / deltaTime : 0;
            });
        }
    ...
    addSeasonalDecomposition(series, metricKey) {
        ...
        return sorted.map(entry => {
            ...
            return {
                ...
                [`${metricKey}_seasonal_residual`]: entry[metricKey] - ((weeklyMean[weekKey] ?? 0) + (monthlyMean[monthKey] ?? 0)) / 2
            };
        });
    }
    ...
    correlationAnalysis(series, xKey, yKey) {
        ...
        const regression = this.trendAnalyzer.linearRegression(values.map((point, index) => ({ x: index, y: point.y })));
        ...
        const correlation = denominatorX > 0 && denominatorY > 0 ? numerator / (denominatorX * denominatorY) : 0;

        const tStatistic = correlation * Math.sqrt((values.length - 2) / Math.max(1 - Math.pow(correlation, 2), 1e-6));
