...
class AdaptationClassifier {
  ...
  #updateCentroids(data, assignments, centroids, keys) {
    centroids.forEach((centroid, clusterIndex) => {
      ...
      keys.forEach(key => {
        centroid[key] =
          clusterPoints.reduce((sum, point) => sum + point[key], 0) / clusterPoints.length;
      });
    });
  }
    ...
    const scores = data.map((point, index) => {
      ...
      const a =
        ownCluster.length > 1
          ? ownCluster
              .filter((_, i) => i !== index)
              .reduce((sum, other) => sum + this.#euclidean(point, other, keys), 0) /
            (ownCluster.length - 1)
          : 0;
      ...
      const bValues = otherClusters
        ...
        .map(
          cluster =>
            cluster.reduce((sum, other) => sum + this.#euclidean(point, other, keys), 0) /
            cluster.length
        );
  ...
  #buildTree(dataset, featureKeys, labelKey, depth) {
    ...
    if (depth === 0 || uniqueLabels.length === 1) {
      const majority = uniqueLabels.sort(
        (a, b) =>
    ...
    featureKeys.forEach(key => {
      ...
      for (let threshold = min + step; threshold < max; threshold += step) {
        ...
        const impurity =
          this.#giniImpurity(left, labelKey) * (left.length / dataset.length) +
