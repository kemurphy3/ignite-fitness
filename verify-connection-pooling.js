...
async function verifyConnectionPooling() {
  ...
  try {
    ...
    console.log(
      `   Status: ${health.neon.healthy && health.pgPool.healthy ? '✅ PASS' : '❌ FAIL'}\n`
    );
    ...
    const result =
      await neonClient`SELECT NOW() as current_time, 'connection-pool-test' as test_name`;
    ...
    const promises = Array.from(
      { length: 10 },
      (_, i) => neonClient`SELECT ${i} as query_id, NOW() as timestamp`
    );
