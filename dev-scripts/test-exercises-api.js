...
async function testExerciseCreation() {
  ...
  const tooManyResult = await apiCall(
    `sessions-${sessionId}-exercises`,
    'POST',
    {
  ...
  const firstResult = await apiCall(
    `sessions-${sessionId}-exercises`,
    'POST',
    {
  ...
  const secondResult = await apiCall(
    `sessions-${sessionId}-exercises`,
    'POST',
    {
      exercises: sameExercises,
...
async function testExerciseListing(authToken, sessionId) {
  ...
  const invalidCursorResult = await apiCall(
    `sessions-${sessionId}-exercises?cursor=invalid`,
    'GET',
    null,
    {
      Authorization: `Bearer ${authToken}`,
...
async function testExerciseUpdates(authToken, sessionId) {
  ...
  const invalidUpdateResult = await apiCall(
    `sessions-${sessionId}-exercises-${exercise.id}`,
    'PUT',
    {
      sets: 0, // Invalid
  ...
  console.log(
    ...
    invalidUpdateResult.status,
    invalidUpdateResult.status === 400 ? '✅' : '❌'
  );
  ...
  const notFoundResult = await apiCall(
    `sessions-${sessionId}-exercises-${fakeId}`,
    'PUT',
    {
      sets: 5,
    },
  ...
  const unauthorizedResult = await apiCall(
    `sessions-${sessionId}-exercises-${exercise.id}`,
    'PUT',
    {
      sets: 5,
...
async function testRateLimiting(authToken, sessionId) {
  ...
  for (let i = 0; i < 65; i++) {
    ...
    promises.push(
      apiCall(`sessions-${sessionId}-exercises`, 'GET', null, {
        Authorization: `Bearer ${authToken}`,
      })
    );
  }
...
async function testValidation() {
  ...
  const tempoResult = await apiCall(
    `sessions-${sessionId}-exercises`,
    'POST',
    {
      exercises: [
  ...
  const invalidTempoResult = await apiCall(
    `sessions-${sessionId}-exercises`,
    'POST',
    {
      exercises: [
  ...
  const validMuscleResult = await apiCall(
    `sessions-${sessionId}-exercises`,
    'POST',
    {
      exercises: [
  ...
  const invalidMuscleResult = await apiCall(
    `sessions-${sessionId}-exercises`,
    'POST',
    {
      exercises: [
  ...
  const longNameResult = await apiCall(
    `sessions-${sessionId}-exercises`,
    'POST',
    {
      exercises: [
  ...
  const longNotesResult = await apiCall(
    `sessions-${sessionId}-exercises`,
    'POST',
    {
      exercises: [
...
async function testSupersetGrouping(authToken, sessionId) {
  ...
  const supersetResult = await apiCall(
    `sessions-${sessionId}-exercises`,
    'POST',
    {
      exercises: [
