...
class ModeSnapshotTest {
    ...
    try {
      ...
      if (hasRPEInput || hasDetailedSettings || hasAdvancedControls) {
        ...
        this.results.failed.push({
          test: 'Advanced Mode snapshot',
          reason: 'Advanced controls missing',
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (simplifiedPlan.circuit && simplifiedPlan.duration === 30) {
        this.results.passed.push(
          'Quick Start: Simplified plan generated (warmup + circuit + finisher)'
        );
      } else {
  ...
  captureUISnapshot(mode) {
    return {
      ...
      advancedControlsVisible: document.querySelectorAll(
        '.advanced-control:not([style*="display: none"])'
      ).length,
