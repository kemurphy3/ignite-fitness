...
Object.assign(window.testPrompts12, {
  ...
  testFeatureFlags() {
    ...
    Object.keys(featureFlags).forEach(flag => {
      console.assert(typeof featureFlags[flag] === 'boolean', `Flag ${flag} should be boolean`);
  ...
  testNeverBlocksCoreSession() {
    ...
    console.assert(canStartWorkout(allFlagsOff) === true, 'Should always allow workout start');
    ...
    console.assert(canStartWorkout(someFlagsOff) === true, 'Should always allow workout start');
  ...
  testSimpleModeNoBreakage() {
    ...
    const simpleModeFeatures = ['start_workout', 'basic_timer', 'record_rpe', 'complete_set'];
