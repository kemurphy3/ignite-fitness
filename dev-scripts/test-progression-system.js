...
async function testProgressionEngine() {
  ...
  try {
    if (typeof window.ProgressionEngine !== 'undefined') {
      ...
      if (
        increaseResult.progression === 'weight_increase' &&
        increaseResult.weight > PROGRESSION_TEST_CONFIG.testExercises.squat.weight
      ) {
      ...
      if (
        decreaseResult.progression === 'weight_decrease' &&
        decreaseResult.weight < PROGRESSION_TEST_CONFIG.testExercises.deadlift.weight
      ) {
        progressionTestResults.passed++;
...
async function testExerciseAdapter() {
  ...
  try {
    if (typeof window.ExerciseAdapter !== 'undefined') {
      ...
      const suggestion = window.ExerciseAdapter.suggestExercise('chest', 'intermediate', [
        'barbell',
      ]);
      ...
      const saveResult = window.ExerciseAdapter.saveExercisePreference(
        'squat',
        'prefer',
        'Great exercise'
      );
...
async function testProgressionAnalytics() {
  ...
  try {
    if (typeof window.ProgressionEngine !== 'undefined') {
      ...
      const preferenceResult = window.ProgressionEngine.saveExercisePreference(
        'squat',
        'prefer',
        'Great exercise'
      );
...
function displayProgressionTestResults() {
  ...
  const passRate =
    totalTests > 0 ? ((progressionTestResults.passed / totalTests) * 100).toFixed(1) : 0;
