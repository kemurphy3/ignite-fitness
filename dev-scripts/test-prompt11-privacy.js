...
Object.assign(window.testPrompts11, {
  ...
  testDataSchema() {
    ...
    tables.forEach(table => {
      ...
      console.log(
        `✅ ${table}: ${schema[table].columns.length} columns, ${schema[table].indexes.length} indexes`
      );
    });
  ...
  testExportJSON() {
    ...
    const mockData = {
      user_profiles: [{ userId: 'user1', username: 'TestUser', sport: 'soccer' }],
      session_logs: [{ userId: 'user1', date: '2024-01-15', workout_id: 'w1', duration: 45 }],
    };
