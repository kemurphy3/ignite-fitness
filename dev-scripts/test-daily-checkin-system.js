  ...
  try {
    if (typeof window.DailyCheckIn !== 'undefined') {
      ...
      if (moderateScore >= 5 && moderateScore <= 7) {
        ...
        dailyCheckInTestResults.failed++;
        dailyCheckInTestResults.errors.push(
          `Moderate readiness score out of range: ${moderateScore}`
        );
      }
...
async function testWorkoutAdjustments() {
  ...
  try {
    if (typeof window.DailyCheckIn !== 'undefined') {
      ...
      if (
        highSorenessAdjustments.recoverySuggested &&
        highSorenessAdjustments.workoutType === 'recovery'
      ) {
        dailyCheckInTestResults.passed++;
...
function displayDailyCheckInTestResults() {
  ...
  const passRate =
    totalTests > 0 ? ((dailyCheckInTestResults.passed / totalTests) * 100).toFixed(1) : 0;
