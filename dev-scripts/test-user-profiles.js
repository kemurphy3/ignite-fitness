...
async function testProfileCreation() {
  ...
  const missingResult = await apiCall(
    'users-profile-post',
    'POST',
    {
      height: 180,
...
async function testProfileUpdates(authToken) {
  ...
  const patchResult = await apiCall(
    'users-profile-patch',
    'PATCH',
    {
  ...
  const versionConflictResult = await apiCall(
    'users-profile-patch',
    'PATCH',
    {
  ...
  console.log(
    ...
    versionConflictResult.status,
    versionConflictResult.status === 409 ? '✅' : '❌'
  );

  // Test 3: Update non-existent field
  console.log('3. Testing invalid field update...');
  const invalidFieldResult = await apiCall(
    'users-profile-patch',
    'PATCH',
    {
      invalid_field: 'test',
  ...
  console.log(
    ...
    invalidFieldResult.status,
    invalidFieldResult.status === 400 ? '✅' : '❌'
  );

  // Test 4: Empty update
  console.log('4. Testing empty update...');
  const emptyUpdateResult = await apiCall(
    'users-profile-patch',
    'PATCH',
    {},
    {
  ...
  console.log(
    'Empty Update Result:',
    emptyUpdateResult.status,
    emptyUpdateResult.status === 400 ? '✅' : '❌'
  );
}
...
async function testProfileValidation(authToken) {
  ...
  const validResult = await apiCall(
  ...
  if (validResult.success) {
    ...
    console.log(
      'Field Errors:',
      Object.keys(invalidResult.data.fields).filter(f => !invalidResult.data.fields[f].valid)
    );
  }

  // Test 3: Conflicting goals
  console.log('3. Testing conflicting goals...');
  const conflictResult = await apiCall(
    'users-profile-validate',
    'POST',
    {
      fields: {
...
async function testSecurity() {
  ...
  const largeResult = await apiCall(
    'users-profile-patch',
    'PATCH',
    {
      goals: largeGoals,
...
async function testUnitConversion() {
  ...
  const imperialResult = await apiCall(
    'users-profile-post',
    'POST',
    {
      age: 25,
  ...
  console.log(
    'Get Imperial Result:',
    getImperialResult.status,
    getImperialResult.success ? '✅' : '❌'
  );
