...
Object.assign(window.testPrompts06, {
  testExerciseSubstitution() {
    ...
    const adapter = {
      suggestSubstitutions(exerciseName, dislikes, painLocation, constraints) {
        ...
        if (painLocation === 'knee') {
          alternatives = alternatives.filter(alt => !alt.name.toLowerCase().includes('squat'));
        }
    ...
    console.assert(result1.alternatives.length > 0, 'Should return alternatives');
    console.assert(
    ...
    const result3 = adapter.suggestSubstitutions('Bulgarian Split Squat', [], 'knee', {});
    console.assert(
  ...
  testPlateMath() {
    ...
    console.assert(us185.plates.length >= 2, 'Should have multiple plates per side');
  ...
  testMissingPlateFallback() {
    ...
    console.assert(fallbackWeight < result.targetWeight, 'Fallback weight should be lower');
  ...
  testSubstitutionUpdatesPlan() {
    ...
    console.assert(newPlan.exercise === 'Walking Lunges', 'Exercise name should update');
    console.assert(newPlan.restTime === 90, 'Rest time should remain same (restAdjustment = 0)');
    console.assert(newPlan.reps === 10, 'Reps should remain same (volumeAdjustment = 1.0)');
    console.assert(newPlan.rationale.length > 0, 'Should include rationale');
