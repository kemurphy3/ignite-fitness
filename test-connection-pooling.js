...
const {
  ...
  healthCheck,
  getStats,
} = require('./netlify/functions/utils/connection-pool');
...
async function testConnectionPooling() {
  ...
  try {
    ...
    const neonPromises = Array.from(
      { length: 50 },
      (_, i) => neonClient`SELECT NOW() as current_time, ${i} as query_id`
    );
    ...
    const stressPromises = [
      ...
      ...Array.from(
        { length: 30 },
        (_, i) => neonClient`SELECT COUNT(*) as count FROM users WHERE id > ${i}`
      ),
      // 20 PG pool queries
    ...
    console.log(
      `   Stress Test (100): ${exhaustionTime}ms (${Math.round(exhaustionTime / 100)}ms avg)`
    );
