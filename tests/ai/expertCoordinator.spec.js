...
(function() {
    ...
    describe('Game Tomorrow Constraint', () => {
        it('should remove heavy lower body work when game is tomorrow', async () => {
            ...
            if (mainBlock) {
                const heavyLowerExercises = mainBlock.items.filter(ex =>
            ...
            expect(plan.why.some(r =>
                r.toLowerCase().includes('game') || r.toLowerCase().includes('tomorrow')
            )).toBe(true);
        });
    });
    ...
    describe('Low Readiness Scaling', () => {
        it('should reduce intensity when readiness is low', async () => {
            ...
            expect(plan.why.some(r =>
                r.toLowerCase().includes('readiness') || r.toLowerCase().includes('recover')
            )).toBe(true);
        });
    });
    ...
    describe('Knee Pain Safe Alternatives', () => {
        it('should not include BSS and provide safe alternatives', async () => {
            ...
            expect(plan.why.some(r =>
                r.toLowerCase().includes('safe') || r.toLowerCase().includes('knee')
            )).toBe(true);
        });
    });
