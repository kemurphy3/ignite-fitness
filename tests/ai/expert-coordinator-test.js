...
const fixtures = {
    normal: {
        ...
        readiness: 8,
        preferences: { aestheticFocus: 'functional', trainingMode: 'simple', availableDays: 4, sessionLength: 45 },
    ...
    gameTomorrow: {
        ...
        readiness: 8,
        preferences: { aestheticFocus: 'functional', trainingMode: 'simple', availableDays: 4, sessionLength: 45 },
        constraints: { timeLimit: 45, equipment: ['barbell', 'dumbbells', 'bench'], flags: ['game_safety'] }
    },
    lowReadiness: {
        user: { sport: 'soccer', position: 'midfielder', weight: 75, height: 180, age: 25 },
        season: 'in-season',
    ...
    timeCrushed: {
        ...
        history: { lastSession: { mainMovement: 'deadlift', averageRPE: 7 } },
        readiness: 8,
        preferences: { aestheticFocus: 'functional', trainingMode: 'simple', availableDays: 4, sessionLength: 20 },
    ...
    kneePain: {
        ...
        readiness: 8,
        preferences: { aestheticFocus: 'functional', trainingMode: 'simple', availableDays: 4, sessionLength: 45 },
        constraints: { timeLimit: 45, equipment: ['barbell', 'dumbbells', 'bench'], flags: ['knee_pain'], painLocation: 'knee' }
    }
};
...
async function runAllTests() {
    ...
    try {
        ...
        if (!plan.why.some(r => r.toLowerCase().includes('game') || r.toLowerCase().includes('tomorrow'))) {
    ...
    try {
        ...
        if (!plan.why.some(r => r.toLowerCase().includes('readiness') || r.toLowerCase().includes('recover'))) {
    ...
    try {
        ...
        if (!plan.why?.some(r => r.toLowerCase().includes('safe') || r.toLowerCase().includes('knee'))) {
