...
describe('ExpertCoordinator Load-Based Adjustments', () => {
  ...
  describe('High-Intensity Yesterday Adjustments', () => {
    it('should suppress heavy lower when Z4 >= 20 minutes', () => {
      ...
      expect(context.loadAdjustments).toContain(
        'Synced HR shows 25 min in Z4 and 5 min in Z5 yesterday → dialing back lower-body volume.'
      );
    });
    ...
    it('should suppress heavy lower when Z5 >= 10 minutes', () => {
      ...
      expect(context.suppressHeavyLower).toBe(true);
      expect(context.loadAdjustments).toContain(
        'Synced HR shows 15 min in Z4 and 12 min in Z5 yesterday → dialing back lower-body volume.'
      );
    });
    ...
    it('should not suppress when Z4 < 20 and Z5 < 10', () => {
      ...
      expect(context.loadAdjustments).not.toContain(
        expect.stringContaining('dialing back lower-body volume')
      );
    });
  });
  ...
  describe('High Strain Adjustments', () => {
    it('should recommend deload when strain > 150', () => {
      ...
      expect(context.loadAdjustments).toContain(
        'High weekly strain detected (180) or monotony > 2.0 → adding mobility emphasis.'
      );
    });
    ...
    it('should recommend deload when monotony > 2.0 and ATL >> CTL', () => {
      ...
      expect(context.recommendDeload).toBe(true);
      expect(context.loadAdjustments).toContain(
        'High weekly strain detected (100) or monotony > 2.0 → adding mobility emphasis.'
      );
    });
    ...
    it('should not recommend deload when conditions not met', () => {
      ...
      expect(context.loadAdjustments).not.toContain(
        expect.stringContaining('adding mobility emphasis')
      );
    });
  });
  ...
  describe('Data Confidence Adjustments', () => {
    it('should enable conservative mode when confidence < 0.5', () => {
      ...
      expect(context.loadAdjustments).toContain(
        'Limited HR data this week (confidence 30%) → conservative recommendation.'
      );
    });
    ...
    it('should not enable conservative mode when confidence >= 0.5', () => {
      ...
      expect(context.loadAdjustments).not.toContain(
        expect.stringContaining('conservative recommendation')
      );
    });
  });
  ...
  describe('Why Panel Explanations', () => {
    it('should generate concrete explanations for load adjustments', () => {
      ...
      expect(context.loadAdjustments).toContain(
        'Synced HR shows 25 min in Z4 and 12 min in Z5 yesterday → dialing back lower-body volume.'
      );
      expect(context.loadAdjustments).toContain(
        'High weekly strain detected (180) or monotony > 2.0 → adding mobility emphasis.'
      );
      expect(
        context.loadAdjustments.some(adj =>
          adj.includes('Rolling load suggests lower readiness → scaling intensity to')
        )
      ).toBe(true);
      expect(context.loadAdjustments).toContain(
        'Limited HR data this week (confidence 30%) → conservative recommendation.'
    ...
    it('should include specific metrics in explanations', () => {
      ...
      expect(context.loadAdjustments).toContain(
        'Synced HR shows 22 min in Z4 and 8 min in Z5 yesterday → dialing back lower-body volume.'
      );
    });
  });
