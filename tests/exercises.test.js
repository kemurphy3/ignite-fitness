...
describe('Exercises API Tests', () => {
  ...
  describe('Exercise Analytics', () => {
    it('should calculate exercise statistics', async () => {
      ...
      const totalWeight = exercises.reduce((sum, ex) => sum + ((ex.weight_kg || 0) * (ex.sets || 0) * (ex.reps || 0)), 0);
      const avgWeight = exercises.filter(ex => ex.weight_kg).reduce((sum, ex) => sum + (ex.weight_kg || 0), 0) / exercises.filter(ex => ex.weight_kg).length;
  ...
  describe('Exercise Performance', () => {
    it('should handle large exercise datasets', async () => {
      ...
      const exercises = Array(20).fill(null).map((_, i) => ({
        name: `Exercise ${i + 1}`,
        sets: 3,
        reps: 10,
        order_index: i
