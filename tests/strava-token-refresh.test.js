...
async function loadHandler({ tokenRecord, fetchImpl, rateLimitAllowed = true, lockAcquired = true } = {}) {
    if (tokenRecord) {
    ...
    vi.mock('../netlify/functions/utils/encryption.js', () => ({
    ...
    vi.mock('../netlify/functions/utils/circuit-breaker.js', () => ({
        stravaCircuit: {
            execute: async fn => fn(),
    ...
    vi.mock('../netlify/functions/utils/distributed-lock.js', () => ({
        acquireRefreshLock: vi.fn(async () => (lockAcquired
    ...
    vi.mock('../netlify/functions/utils/rate-limiter.js', () => ({
        checkEndpointRateLimit: vi.fn(async () => (rateLimitAllowed
            ? { allowed: true }
            : { allowed: false, reason: 'LIMIT', retryAfter: 30 })),
        getRateLimitHeaders: vi.fn(() => ({}))
    }), { virtual: true });

    vi.mock('../netlify/functions/utils/audit.js', () => ({
        auditLog: vi.fn(async () => {})
    }), { virtual: true });

    vi.mock('../netlify/functions/utils/safe-logging.js', () => ({
        createLogger: () => ({
            tokenRefresh: vi.fn(),
            error: vi.fn(),
            warn: vi.fn(),
