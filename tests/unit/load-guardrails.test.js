...
class LoadGuardrails {
    ...
    initializeRampRateThresholds() {
        return {
            beginner: { maxWeeklyIncrease: 0.08, hiitReduction: 0.25, consecutiveDaysLimit: 3, minRestDays: 2 },
            intermediate: { maxWeeklyIncrease: 0.10, hiitReduction: 0.20, consecutiveDaysLimit: 4, minRestDays: 1 },
            advanced: { maxWeeklyIncrease: 0.12, hiitReduction: 0.15, consecutiveDaysLimit: 5, minRestDays: 1 },
            elite: { maxWeeklyIncrease: 0.15, hiitReduction: 0.10, consecutiveDaysLimit: 6, minRestDays: 1 }
        };
    }
    ...
    initializeRecoveryProtocols() {
        return {
            ...
            missedDays: { action: 'gradual_return', rampDown: 0.15, maxReduction: 0.40 },
            painFlag: { action: 'immediate_downshift', reduction: 0.30, duration: 14 },
    ...
    async handlePainFlag(userId, painLevel, location) {
        ...
        return { status: 'pain_response_applied', actions: [{ type: 'immediate_downshift', reduction: scaledReduction }] };
    }
    ...
    async getWeeklyLoadHistory(userId, weeks) { return []; }
    async getRecentSessions(userId, days) { return []; }
    async getUpcomingSessions(userId, days) { return []; }
    ...
    async getActiveAdjustments(userId) { return []; }
    async applyRampRateGuardrails(userId, rampAnalysis, thresholds) { return []; }
    ...
    calculateRecommendedReduction(rampRate, thresholds) { return 0.2; }
    generateRampRateMessage(rampRate, thresholds, severity) { return 'Test message'; }
    countConsecutiveTrainingDays(sessions) { return 0; }
    sessionViolatesAdjustment(session, adjustment) { return false; }
    getSessionIntensity(session) { return 0.6; }
    async getUserSessions(userId) { return []; }
    calculateNextReviewDate() { return new Date().toISOString(); }
    async getGuardrailStatus(userId) { return { activeAdjustments: [], recentAnalysis: {}, isUnderGuardrail: false }; }
}
...
describe('LoadGuardrails', () => {
    ...
    describe('Initialization', () => {
        it('should initialize with correct ramp rate thresholds', () => {
            ...
            expect(guardrails.rampRateThresholds.intermediate.maxWeeklyIncrease).toBe(0.10);
            expect(guardrails.rampRateThresholds.intermediate.hiitReduction).toBe(0.20);
        });
    ...
    describe('Ramp Rate Monitoring', () => {
        it('should detect ramp rate exceeding threshold', async () => {
            ...
            expect(result.rampRate).toBeGreaterThan(0.10);
        ...
        it('should allow ramp rate within threshold', async () => {
            ...
            expect(result.rampRate).toBeLessThanOrEqual(0.10);
        });

        it('should return insufficient data for < 2 weeks', async () => {
            const loadHistory = [
                { week: 0, totalLoad: 200, startDate: new Date().toISOString() }
            ];
        ...
        it('should cap HIIT reduction at 50%', () => {
            const rampAnalysis = {
                rampRate: 0.50, // 50% increase (very high)
                exceedsThreshold: true,
                severity: 'high'
            };
    ...
    describe('HIIT Reduction', () => {
        ...
        it('should modify upcoming HIIT sessions', async () => {
            ...
            await guardrails.modifyUpcomingHIIT('testuser', 0.20);
        ...
        it('should reduce intensity zones correctly', () => {
            expect(guardrails.reduceIntensityZone('Z5', 0.20)).toBe('Z4');
            expect(guardrails.reduceIntensityZone('Z4', 0.30)).toBe('Z3');
            expect(guardrails.reduceIntensityZone('Z3', 0.20)).toBe('Z2');
        });
    });
    ...
    describe('Missed Days Handling', () => {
        ...
        it('should cap reduction at 40% for missed days', async () => {
            ...
            expect(result.actions[0].reduction).toBeLessThanOrEqual(0.40);
        });
    });
    ...
    describe('Pain Flag Handling', () => {
        it('should apply immediate downshift for pain', async () => {
            ...
            expect(result.actions[0].reduction).toBeGreaterThanOrEqual(0.30);
        });
    ...
    describe('Consecutive Increases Detection', () => {
        it('should detect consecutive weeks of high increases', () => {
            ...
            const consecutive = guardrails.checkConsecutiveIncreases(loadHistory, 0.10);
        ...
        it('should trigger deload week recommendation for consecutive increases', async () => {
            ...
            guardrails.applyRampRateGuardrails = vi.fn().mockResolvedValue([
                { type: 'reduce_hiit', reduction: 0.20 },
    ...
    describe('Edge Cases - Enhanced', () => {
        it('should handle zero previous load without crashing', () => {
            ...
            const rampRate = guardrails.loadCalculator?.calculateRampRate?.(currentPeriod, previousPeriod) || 0;
    ...
    describe('Performance', () => {
        it('should handle large load history efficiently', async () => {
            ...
            const analysis = guardrails.analyzeRampRate(0.15, { maxWeeklyIncrease: 0.10 }, largeHistory);
