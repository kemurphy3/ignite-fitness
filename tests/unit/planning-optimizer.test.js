...
describe('PeriodizationEngine', () => {
  it('builds multi-phase plans with progressive intensity', () => {
    ...
    const goals = [{ type: 'strength' }, { type: 'endurance' }];
    ...
    const accumulationIntensity =
      plan.phases[0].weeklyPlan.reduce((sum, session) => sum + session.intensity, 0) /
      plan.phases[0].weeklyPlan.length;
    const intensificationIntensity =
      plan.phases[1].weeklyPlan.reduce((sum, session) => sum + session.intensity, 0) /
      plan.phases[1].weeklyPlan.length;
...
describe('ProgressionCalculator', () => {
  it('generates progression models and schedules deloads', () => {
    ...
    expect(model.doubleProgression[0].weightProgression[0]).toBeLessThan(
      model.doubleProgression[0].weightProgression[3]
    );
