...
describe('Beta Database Acceptance Tests', () => {
  ...
  describe('Workout Catalog Access', () => {
    it('should fetch non-empty catalogs for all 3 modalities', async () => {
      ...
      console.log(
        `✅ Workout catalogs verified: ${runningWorkouts.length} running, ${cyclingWorkouts.length} cycling, ${swimmingWorkouts.length} swimming`
      );
    });
  ...
  describe('Substitution Rules Verification', () => {
    it('should query substitution rules for Run→Bike Z2 and return scale ~1.3±0.05', async () => {
      ...
      console.log(
        `✅ Run→Bike Z2 substitution verified: factor=${rule.time_factor}, confidence=${rule.confidence_score}`
      );
    });
    ...
    it('should have modality conversion factors', async () => {
      ...
      const runToCycle = factors.find(
        f => f.from_modality === 'running' && f.to_modality === 'cycling'
      );
      const cycleToRun = factors.find(
        f => f.from_modality === 'cycling' && f.to_modality === 'running'
      );
  ...
  describe('Guardrails Configuration', () => {
    ...
    it('should have progressive guardrail restrictions by level', async () => {
      ...
      expect(beginner.max_weekly_load_increase).toBeLessThanOrEqual(0.1);
    });
