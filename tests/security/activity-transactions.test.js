...
describe('Activity Transaction Manager', () => {
      ...
      class MockActivityTransactionManager {
        ...
        async executeActivityDedupTransaction(normalized, userId, affectedDates) {
          ...
          try {
            ...
            const existingActivity = await this.findActivityByDedupHash(
              normalized.dedupHash,
              userId
            );

            if (existingActivity) {
              return await this.handleExistingActivityInTransaction(
                existingActivity,
                normalized,
                userId,
                affectedDates,
              ...
              const likelyDuplicates = await this.findLikelyDuplicatesInTransaction(
                normalized,
                userId
              );

              if (likelyDuplicates.length > 0) {
                return await this.handleLikelyDuplicateInTransaction(
                  likelyDuplicates[0],
                  normalized,
                  userId,
                  affectedDates,
                ...
                return await this.handleNewActivityInTransaction(
                  normalized,
                  userId,
                  affectedDates,
                  transactionId
        ...
        async handleExistingActivityInTransaction(
          existing,
          normalized,
          userId,
          affectedDates,
        ...
        async handleLikelyDuplicateInTransaction(
          existing,
          normalized,
          userId,
          affectedDates,
        ...
        async handleNewActivityInTransaction(normalized, userId, affectedDates, transactionId) {
          ...
          return {
            ...
            externalId: normalized.canonicalExternalId,
            status: 'imported',
            richness: this.calculateRichness(normalized.rawActivity),
          };
        }
        ...
        async attachStreamsInTransaction(streamsByActivityId, activitiesById, transactionId) {
          for (const [externalId, streams] of Object.entries(streamsByActivityId)) {
            const activity = Array.from(activitiesById.values()).find(
        ...
        calculateRichness(activity) {
          ...
          if (activity.start_latlng || activity.distance) {
            score += 0.2;
          }
  ...
  describe('Stream Attachment Transactions', () => {
    it('should attach streams atomically', async () => {
      const activitiesById = new Map([[1, { id: 1, externalId: '123' }]]);
    ...
    it('should rollback stream attachment on failure', async () => {
      ...
      await expect(
        transactionManager.attachStreamsInTransaction(
          streamsByActivityId,
          activitiesById,
          'streams_tx'
        )
