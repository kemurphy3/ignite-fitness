...
class ExpertCoordinatorTest {
    ...
    testGameMinus1Day() {
        ...
        const hasHeavyLegWork = plan.mainSets.some(main => 
    ...
    testKneePainConflict() {
        ...
        const hasGobletSquat = plan.mainSets.some(main => 
            main.exercise === 'goblet_squat' || 
            main.rationale?.toLowerCase().includes('knee')
        );
        
        if (hasGobletSquat) {
            this.results.passed++;
            this.results.tests.push({ name: 'Knee pain: Squat substituted with goblet', status: '✅ PASS' });
        } else {
            this.results.failed++;
            this.results.tests.push({ name: 'Knee pain: Squat substituted with goblet', status: '❌ FAIL' });
        }
    }
    ...
    testLowReadiness() {
        ...
        const hasVolumeReduction = plan.notes.some(note => 
            note.text?.toLowerCase().includes('reduced volume') || 
            note.text?.toLowerCase().includes('low readiness')
        );
        
        const hasRecoveryFocus = plan.sessionNotes?.toLowerCase().includes('readiness') &&
                                  plan.sessionNotes?.toLowerCase().includes('3');
        
        if (hasVolumeReduction || hasRecoveryFocus) {
            this.results.passed++;
            this.results.tests.push({ name: 'Low readiness: Volume reduced, recovery focus', status: '✅ PASS' });
        } else {
            this.results.failed++;
            this.results.tests.push({ name: 'Low readiness: Volume reduced, recovery focus', status: '❌ FAIL' });
        }
    }
    ...
    testAestheticGlutesFocus() {
        ...
        const hasGluteAccessories = plan.accessories.some(acc => 
        ...
        if (hasGluteAccessories) {
            this.results.passed++;
            this.results.tests.push({ name: 'Glutes focus: Hip thrusts and glute accessories added', status: '✅ PASS' });
        } else {
            this.results.failed++;
            this.results.tests.push({ name: 'Glutes focus: Hip thrusts and glute accessories added', status: '❌ FAIL' });
        }
    }
    ...
    testMultipleConflicts() {
        ...
        if (hasSubstitutions && hasNotes && hasRationale) {
            this.results.passed++;
            this.results.tests.push({ name: 'Multiple conflicts: All constraints respected', status: '✅ PASS' });
        } else {
            this.results.failed++;
            this.results.tests.push({ name: 'Multiple conflicts: All constraints respected', status: '❌ FAIL' });
        }
    }
    ...
    testRationaleGeneration() {
        ...
        if (allShort && allValid) {
            this.results.passed++;
            this.results.tests.push({ name: 'Rationale: 1-2 sentences, renderable in UI', status: '✅ PASS' });
        } else {
            this.results.failed++;
            this.results.tests.push({ name: 'Rationale: 1-2 sentences, renderable in UI', status: '❌ FAIL' });
        }
    }
