...
Object.assign(window.testPrompts08, {
    ...
    testPassiveReadinessInference() {
        ...
        console.assert(result3.score < 6, 'Should significantly reduce readiness after multiple hard days');
    ...
    testExternalLoadAdaptation() {
        ...
        const hasLegWork = workout.exercises.some(ex => 
            ['squat', 'lunge', 'deadlift'].some(keyword => 
        ...
        if (hasLegWork && externalActivity.type === 'running') {
            ...
            console.log(`✅ Adapted squat: ${workout.exercises[0].sets} → ${adaptedWorkout.exercises[0].sets} sets`);
            console.log(`✅ Adapted RDL: ${workout.exercises[1].sets} → ${adaptedWorkout.exercises[1].sets} sets`);
            console.log(`✅ Adapted split squat: ${workout.exercises[2].sets} → ${adaptedWorkout.exercises[2].sets} sets`);
    ...
    testPlansAdaptWithoutDailyInput() {
        ...
        console.assert(
            inferredReadiness.score < 7,
            'Should infer low readiness from data'
        );
        ...
        console.log(`✅ Adapted: intensity ${Math.round(adaptedPlan.intensity)}%, volume ${Math.round(adaptedPlan.volume)}`);
