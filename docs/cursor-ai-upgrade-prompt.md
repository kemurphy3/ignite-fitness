...
Update the ignite-fitness tracker (tracker.html) to implement a sophisticated
multi-expert AI model that balances perspectives from different fitness
professionals for optimal soccer performance and aesthetics.
...
const EXPERT_SYSTEM = {
  weights: {
    soccerStrengthCoach: 0.3, // Programming, periodization, game readiness
    physicalTherapist: 0.25, // Injury prevention, movement quality, recovery
    personalTrainer: 0.2, // Progressive overload, exercise form
    eliteSoccerCoach: 0.1, // Performance transfer, energy systems
    nutritionist: 0.1, // Recovery nutrition, game-day fueling
...
```javascript
function calculateSmartWeightProgression(
  exerciseName,
  currentWeight,
  exerciseType,
  performance
...
function calculateBarbellProgression(currentWeight, performance) {
  ...
  const sortedWeights = Object.keys(plateCombos)
    .map(Number)
    .sort((a, b) => a - b);
...
Update the `getGoalOrientedExercises` function (around line 3507) to ALWAYS
include core:
...
function selectExercisesWithExpertConsensus(sessionType, goals, week) {
  ...
  const sorted = Object.values(exerciseScores).sort(
    (a, b) => b.score - a.score
  );
  ...
  return [
    ...mainExercises.map(e => e.exercise),
    ...accessories.map(e => e.exercise),
  ];
}
```
...
This will show which expert contributed each exercise, helping users understand
the reasoning.
...
<div class="rpe-input" style="margin-top: 10px;">
  <label style="font-size: 12px; color: #a0aec0;"
    >How hard was this? (RPE)</label
  >
  <div style="display: flex; gap: 5px; margin-top: 5px;">
    <button onclick="setRPE(this, 6)" class="rpe-btn">
      6<br /><span style="font-size: 10px;">Easy</span>
    </button>
    <button onclick="setRPE(this, 7)" class="rpe-btn">
      7<br /><span style="font-size: 10px;">Moderate</span>
    </button>
    <button onclick="setRPE(this, 8)" class="rpe-btn">
      8<br /><span style="font-size: 10px;">Hard</span>
    </button>
    <button onclick="setRPE(this, 9)" class="rpe-btn">
      9<br /><span style="font-size: 10px;">V.Hard</span>
    </button>
    <button onclick="setRPE(this, 10)" class="rpe-btn">
      10<br /><span style="font-size: 10px;">Max</span>
    </button>
  </div>
  <small style="color: #68d391;"
    >Next recommended: <span class="next-weight">Calculating...</span></small
  >
</div>
...
function calculatePersonalizedMacros(userData) {
  ...
  const totalSessions =
    parseInt(soccerFrequency || 0) + parseInt(liftingFrequency || 0);
  let activityMultiplier;

  if (totalSessions <= 2) activityMultiplier = activityMultipliers.light;
  else if (totalSessions <= 4)
    activityMultiplier = activityMultipliers.moderate;
...
function calculateMacroSplit(calories, bodyweight, goals, soccerFrequency) {
  ...
  const expertRecommendations = {
    soccerCoach: {
      ...
      carbPercent: 0.5, // High for performance
      fatPercent: 0.25,
    },
    strengthCoach: {
  ...
  if (
    ...
    goals.secondary?.includes('aesthetics')
  ) {
...
function generateNutritionNotes(goals, soccerFrequency) {
  ...
  if (
    goals.primary === 'acl_prevention' ||
    goals.secondary?.includes('injury_prevention')
  ) {
...
function recalculateMacros() {
  ...
  if (avgEnergy < 6 && weightTrend === 'stable') {
    ...
    weightTrend === 'gaining_fast' &&
    userData.goals.primary !== 'muscle_gain'
  ) {
