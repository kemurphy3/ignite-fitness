...
npm automatically removes duplicate packages by "hoisting" shared dependencies
to the root `node_modules`. When you update packages (like vitest 1.0 → 2.0),
npm rebuilds the dependency tree and removes duplicates.
...
npm tries to minimize `node_modules` size by moving packages up the tree. The
total count went:
...
Your app functionality is unchanged - npm just optimized the dependency tree.
Think of it like cleaning up duplicate files on your hard drive!
