...
Implement a context-aware AI system that includes user history in every LLM
call, making the AI feel like it "remembers" users without actual learning. This
is the sweet spot for cost vs intelligence.
...
${context.recentWorkouts
  .map(w => `- ${w.date}: ${w.type}, RPE ${w.rpe}, ${w.completed ? '✓' : '✗'}`)
  .join('\n')}
...
Remember:
- They respond well to: ${context.patterns
      .filter(p => p.positive)
      .map(p => p.factor)
      .join(', ')}
- They struggle with: ${context.patterns
      .filter(p => !p.positive)
      .map(p => p.factor)
      .join(', ')}
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are an expert fitness coach specializing in hybrid athletes who both lift weights and play sports.',
          },
          {
            role: 'user',
            content: prompt,
...
class PatternDetector {
  async detectPatterns(userId) {
    ...
    exercisePatterns.forEach(pattern => {
      if (pattern.improvementRate > 0.05) {
        // 5% improvement
        patterns.push({
      ...
      } else if (pattern.improvementRate < -0.02) {
        // Regression
        patterns.push({
          type: 'exercise_response',
    ...
    days.forEach(day => {
      ...
      if (dayWorkouts.length > 0) {
        dayStats[day] = {
          ...
          completion:
            dayWorkouts.filter(w => w.completed).length / dayWorkouts.length,
        };
      }
    });
...
class ModelSelector {
  ...
  isFollowUp(question, context) {
    ...
    return (
      (context.conversationHistory?.length > 0 && question.includes('it')) ||
      question.includes('that') ||
  ...
  logUsage(model, actualTokens, responseQuality) {
    ...
    console.log(
      `Model: ${model}, Tokens: ${actualTokens}, Quality: ${responseQuality}`
    );
  }
}
```
...
async function sendToAI() {
  ...
  try {
    ...
    if (error.message.includes('rate limit')) {
      const fallbackResponse = await contextAwareAI.askAI(
        currentUser.id,
        question,
        {
...
class RAGSystem {
  ...
  async askAIWithRAG(userId, question) {
    ...
    const ragPrompt = `
      ...
      ${similarPatterns
        .map(
          p => `- User with similar profile: ${p.workout} led to ${p.outcome}`
        )
        .join('\n')}
...
async function trackAPIUsage(userId, model, tokens) {
  ...
  if (monthlyUsage > monthlyPayment * 0.8) {
    console.warn(
      `User ${userId} approaching cost limit: $${monthlyUsage}/$${monthlyPayment}`
    );
  }
}
```
