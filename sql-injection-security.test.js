...
async function runTests() {
  ...
  for (const payload of SQL_INJECTION_PAYLOADS) {
    ...
    try {
      ...
      const { status, data } = await makeRequest(
        `users-profile-get?external_id=${encodeURIComponent(payload)}`,
        userToken
      );
  ...
  for (const payload of SQL_INJECTION_PAYLOADS) {
    ...
    try {
      ...
      const { status, data } = await makeRequest(
        `sessions-list?type=${encodeURIComponent(payload)}`,
        userToken
      );
  ...
  for (const payload of SQL_INJECTION_PAYLOADS) {
    ...
    try {
      ...
      const { status, data } = await makeRequest(
        `admin-sessions-by-type?from=${encodeURIComponent(payload)}`,
        adminToken
      );
  ...
  for (const payload of SQL_INJECTION_PAYLOADS) {
    ...
    try {
      ...
      const { status, data } = await makeRequest(
        `sessions-exercises-list?session_id=${encodeURIComponent(payload)}`,
        userToken
      );
  ...
  for (const payload of SQL_INJECTION_PAYLOADS) {
    ...
    try {
      ...
      const { status, data } = await makeRequest(
        `integrations-strava-status?user_id=${encodeURIComponent(payload)}`,
        userToken
      );
  ...
  if (failed === 0) {
    ...
    console.log(
      '   Please review the implementation and ensure all queries use parameterized statements.'
    );
  }
