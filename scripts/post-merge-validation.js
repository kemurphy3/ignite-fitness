...
class PostMergeValidator {
    constructor() {
        ...
        this.fetch = typeof globalThis.fetch === 'function'
    ...
    validateSubstitutionObject(substitution, index) {
        ...
        if (typeof substitution.adaptationMatch === 'number' &&
            substitution.adaptationMatch >= 0 && substitution.adaptationMatch <= 1) {
    ...
    async testSoccerShapeContent() {
        try {
            const response = await this.fetchWithFallback(`${this.baseUrl}/api/workouts?category=soccer_shape`, {
                signal: this.createTimeoutSignal(5000)
            });
    ...
    generateReport() {
        ...
        this.log(`\n🎯 DEPLOYMENT READY: ${deploymentReady ? 'YES' : 'NO'}`, deploymentReady ? 'green' : 'red');
