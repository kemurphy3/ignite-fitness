...
class AccessibilityRegressionChecker {
  ...
  comparePa11yResults() {
    ...
    const newIssues = currentIssues.filter(
      currentIssue =>
        !baselineIssues.some(baselineIssue => this.issuesEqual(currentIssue, baselineIssue))
    );

    // Check for resolved issues
    const resolvedIssues = baselineIssues.filter(
      baselineIssue =>
        !currentIssues.some(currentIssue => this.issuesEqual(currentIssue, baselineIssue))
    );
   ...
   */
  saveCriticalIssues() {
    const criticalIssues = this.regressions.filter(regression => regression.severity === 'high');
