...
function checkCoreWebVitals(audits) {
    const metrics = {
        'largest-contentful-paint': { value: audits['largest-contentful-paint']?.numericValue, budget: BUDGETS.lcp },
        'first-input-delay': { value: audits['first-input-delay']?.numericValue, budget: BUDGETS.fid },
        'cumulative-layout-shift': { value: audits['cumulative-layout-shift']?.numericValue, budget: BUDGETS.cls },
        'server-response-time': { value: audits['server-response-time']?.numericValue, budget: BUDGETS.ttfb },
...
function reportResults() {
    ...
    Object.entries(groupedViolations).forEach(([type, violations]) => {
        ...
        violations.forEach(violation => {
            console.log(`  ❌ ${violation.metric}: ${violation.current} (budget: ${violation.budget}, excess: ${violation.excess})`);
        });
        console.log('');
    });
