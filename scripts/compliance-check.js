...
class ComplianceChecker {
  ...
  async checkEncryptionInTransit() {
    try {
      ...
      const hasSecurityHeaders = this.checkFileExists(
        'netlify/functions/utils/security-headers.js'
      );
  ...
  async checkDatabaseConfiguration() {
    try {
      ...
      const hasConnectionPooling = this.checkFileExists(
  ...
  calculateComplianceScore() {
    ...
    // Count all checks
    const allChecks = [this.results.gdpr, this.results.security, this.results.dataHandling];
    ...
    this.logger.log(
      `📊 Compliance Score: ${this.results.score}% (${passedChecks}/${totalChecks} checks passed)`
    );
  }
  ...
  checkComplianceThresholds() {
    ...
    return (
      this.results.score >= thresholds.minComplianceScore &&
