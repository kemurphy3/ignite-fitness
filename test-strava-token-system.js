...
async function testOAuthExchange() {
  ...
  console.log(
    ...
    invalidMethodResult.status,
    invalidMethodResult.status === 405 ? '✅' : '❌'
  );
}
...
async function testTokenRefresh() {
  ...
  console.log(
    'Missing User ID Result:',
    missingUserIdResult.status,
    missingUserIdResult.status === 400 ? '✅' : '❌'
  );
...
async function testCircuitBreaker() {
  ...
  for (let i = 0; i < 10; i++) {
    promises.push(
      apiCall('strava-oauth-exchange', 'POST', {
        code: 'invalid-code',
        userId: TEST_CONFIG.userId,
      })
    );
...
async function testConcurrentRefresh() {
  ...
  for (let i = 0; i < 5; i++) {
    promises.push(
      apiCall('strava-refresh-token', 'POST', {
        userId: TEST_CONFIG.userId,
      })
    );
  }
...
async function testCaching() {
  ...
  console.log(
    'First Request Cache:',
    firstResult.headers['x-cache'],
    firstResult.headers['x-cache'] === 'MISS' ? '✅' : '❌'
  );
  ...
  console.log(
    ...
    secondResult.headers['x-cache'],
    secondResult.headers['x-cache'] === 'HIT' ? '✅' : '❌'
  );
}
...
async function testSecurityHeaders() {
  ...
  const presentHeaders = securityHeaders.filter(
    header => result.headers[header] || result.headers[header.toLowerCase()]
  );
