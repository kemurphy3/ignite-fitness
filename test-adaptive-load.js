...
test('Readiness ≤ 4 should swap to recovery session', () => {
  ...
  if (readinessScore > 4 || adjustments.workoutType !== 'recovery') {
    throw new Error(
      `Expected recovery session, got: ${adjustments.workoutType}, readiness: ${readinessScore}`
    );
  }
});
...
test('Readiness 5-7 should reduce intensity by 10%', () => {
  ...
  if (Math.abs(adjustments.intensityMultiplier - 0.9) > 0.01) {
...
test('Every 4th week should reduce volume by 20%', () => {
  ...
  if (Math.abs(deload.volumeMultiplier - 0.8) > 0.01) {
...
test('RPE <5 should increase load by 10%', async () => {
  ...
  if (Math.abs(adjustments.intensityMultiplier - 1.1) > 0.01) {
