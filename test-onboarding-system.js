...
async function testOnboardingModule() {
  ...
  try {
    if (typeof window.OnboardingManager !== 'undefined') {
      ...
      const updateResult = window.OnboardingManager.updateUserPreferences(
        ONBOARDING_TEST_CONFIG.testPreferences
      );
...
function displayOnboardingTestResults() {
  ...
  const passRate =
    totalTests > 0 ? ((onboardingTestResults.passed / totalTests) * 100).toFixed(1) : 0;
