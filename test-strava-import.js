...
async function testStravaImport() {
    ...
    const noConnectionResult = await apiCall('integrations-strava-import', 'POST', {}, {
        'Authorization': `Bearer ${authToken}`
    });
    
    console.log('No Connection Result:', noConnectionResult.status, noConnectionResult.status === 403 ? '✅' : '❌');
    ...
    const invalidAfterResult = await apiCall('integrations-strava-import?after=99999999999', 'POST', {}, {
        'Authorization': `Bearer ${authToken}`
    });
    
    console.log('Invalid After Result:', invalidAfterResult.status, invalidAfterResult.status === 400 ? '✅' : '❌');

    // Test 3: Invalid per_page parameter
    console.log('3. Testing invalid per_page parameter...');
    const invalidPerPageResult = await apiCall('integrations-strava-import?per_page=200', 'POST', {}, {
        'Authorization': `Bearer ${authToken}`
    });
    
    console.log('Invalid Per Page Result:', invalidPerPageResult.status, invalidPerPageResult.status === 400 ? '✅' : '❌');
...
async function testContinueToken() {
    ...
    const { generateContinueToken, parseContinueToken } = require('./netlify/functions/utils/strava-import');
...
async function testURLBuilding() {
    ...
    console.log('Basic URL Test:', basicUrl.includes('page=1') && basicUrl.includes('after=1704067200') ? '✅' : '❌');
    ...
    console.log('Cursor URL Test:', cursorUrl.includes('before=123456789') && !cursorUrl.includes('page=') ? '✅' : '❌');
