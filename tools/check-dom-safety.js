...
function checkJSFile(filePath) {
    try {
        ...
        lines.forEach((line, index) => {
            ...
            const unsafePattern = /getElementById\([^)]+\)\.(innerHTML|textContent|value|classList|style|disabled|checked)/;
            if (unsafePattern.test(line)) {
                ...
                const context = content.substring(Math.max(0, functionStart), content.indexOf(line) + line.length);
                ...
                const hasGuard = new RegExp(`getElementById\\(['"]${elementId}['"]\\)\\s*(&&|\\?|if)`).test(context) ||
                                new RegExp(`const\\s+\\w+\\s*=\\s*getElementById\\(['"]${elementId}['"]\\)`).test(context);
            ...
            const unsafeQueryPattern = /querySelector\([^)]+\)\.(innerHTML|textContent|value|classList|style|disabled|checked|appendChild|removeChild)/;
            ...
            if (line.includes('<script>') || (line.includes('function') && !line.includes('src='))) {
                ...
                for (let i = index; i < Math.min(lines.length, index + 100); i++) {
                    ...
                    if (inScript) {
                        ...
                        const unsafePattern = /getElementById\([^)]+\)\.(innerHTML|textContent|value|classList)/;
                        if (unsafePattern.test(currentLine)) {
                            ...
                            if (idMatch) {
                                ...
                                const hasGuard = new RegExp(`getElementById\\(['"]${elementId}['"]\\)\\s*(&&|\\?|if|const\\s+\\w+\\s*=)`).test(scriptContent);
