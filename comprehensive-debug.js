...
function testDOMElements() {
  ...
  logTest(
    ...
    missingElements.length > 0 ? `Missing: ${missingElements.join(', ')}` : null
  );
}
...
function testJavaScriptFunctions() {
  ...
  logTest(
    ...
    missingFunctions.length > 0 ? `Missing: ${missingFunctions.join(', ')}` : null
  );
}
...
function testCSSClasses() {
  ...
  logTest(
    ...
    missingClasses.length > 0 ? `Missing: ${missingClasses.join(', ')}` : null
  );
}
...
function testFormValidation() {
  try {
    ...
    const hasError =
      ...
      errorDiv.textContent.includes('Please enter both username and password');
...
function testTabNavigation() {
  try {
    ...
    logTest(
      'Tab Navigation',
      tabButtons.length > 0 && tabContents.length > 0,
      tabButtons.length === 0
        ? 'No tab buttons found'
        : tabContents.length === 0
          ? 'No tab contents found'
          : null
    );
  } catch (error) {
...
function testLocalStorage() {
  try {
    ...
    logTest(
      'LocalStorage',
      retrieved === testValue,
      retrieved !== testValue ? 'Retrieved value does not match stored value' : null
    );
  } catch (error) {
...
function testJSONParsing() {
  try {
    ...
    logTest(
      'JSON Parsing',
      JSON.stringify(parsedData) === jsonString,
...
function testPasswordHashing() {
  try {
    ...
    logTest(
      ...
      hash1 !== hash2 ? 'Hashes are not consistent' : hash1.length === 0 ? 'Hash is empty' : null
    );
  } catch (error) {
...
function testErrorHandling() {
  try {
    ...
    const hasError =
      testDiv.textContent === 'Test error message' && testDiv.style.display === 'block';
...
function testSuccessNotifications() {
  try {
    ...
    logTest(
      ...
      hasNotification,
      hasNotification ? null : 'Success notification not created'
    );
  } catch (error) {
...
function testRadioButtonSelection() {
  try {
    ...
    logTest(
      ...
      primaryGoalInputs.length > 0 && secondaryGoalInputs.length > 0,
      primaryGoalInputs.length === 0
        ? 'No primary goal inputs found'
        : secondaryGoalInputs.length === 0
          ? 'No secondary goal inputs found'
          : null
    );
  } catch (error) {
...
function testFormInputTypes() {
  try {
    ...
    logTest(
      'Form Input Types',
      hasCorrectTypes,
      hasCorrectTypes ? null : 'Missing required input types'
    );
  } catch (error) {
