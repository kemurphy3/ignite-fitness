...
Object.assign(window.testPrompts07, {
  ...
  testChipRendering() {
    ...
    console.assert(rendered.className === 'why-this-chip', 'Should have correct class');
    console.assert(rendered.innerHTML.includes('Why?'), 'Should show "Why?" label');
    console.assert(rendered.innerHTML.includes(reason), 'Should include reason text');
    console.assert(rendered.innerHTML.includes('aria-expanded="false"'), 'Should start collapsed');
  ...
  testReasonLogging() {
    ...
    console.assert(mockLog.events.length === 1, 'Should log event');
    console.assert(
      mockLog.events[0].eventType === 'WHY_REASON_VIEWED',
      'Should have correct event type'
    );
    console.assert(mockLog.events[0].reason === event.reason, 'Should include reason');
    console.assert(
  ...
  testModeRendering() {
    ...
    const blocks = [{ exercise: 'Warmup' }, { exercise: 'Back Squat' }, { exercise: 'Accessory' }];
