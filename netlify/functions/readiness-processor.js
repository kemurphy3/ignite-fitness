...
function checkScheduleConflicts(data) {
  ...
  if (gameDates && gameDates.length > 0) {
    gameDates.forEach(gameDate => {
      const daysUntil = Math.ceil(
        (new Date(gameDate) - new Date(workoutDate)) / (1000 * 60 * 60 * 24)
      );
  ...
  if (previousSessions && previousSessions.length > 0) {
    ...
    const daysSince = Math.ceil(
      (new Date(workoutDate) - new Date(lastSession.date)) / (1000 * 60 * 60 * 24)
    );
  ...
  return {
    ...
    recommendation:
      conflicts.length > 0 ? conflicts[0].recommendation : 'Proceed with scheduled workout',
  };
}
