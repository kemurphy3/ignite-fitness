...
exports.handler = async event => {
  ...
  try {
    ...
    if (!token) {
      return errorResponse(401, 'MISSING_TOKEN', 'Authorization required');
    }
    ...
    if (table === 'user_profiles') {
      ...
      const date =
        parts.length > 1 ? parts.slice(1).join('_') : data.date || data.logged_at || null;
      if (!date) {
        return errorResponse(422, 'INVALID_KEY', 'Date missing in key');
      }
