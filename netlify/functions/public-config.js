...
exports.handler = async (event, _context) => {
  ...
  try {
    ...
    if (
      !stravaClientId ||
      String(stravaClientId).trim() === '' ||
      String(stravaClientId).toLowerCase() === 'undefined'
    ) {
    ...
    const publicConfig = {
      ...
      integrations: {
        strava: {
          ...
          redirectUri:
            process.env.STRAVA_PUBLIC_REDIRECT_URI ||
            `${process.env.NETLIFY_URL || 'http://localhost:8888'}/strava-callback.html`,
