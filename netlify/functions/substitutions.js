...
const EquivalenceRules = {
  getTimeFactor(fromModality, toModality, zone) {
    ...
    const baseFactors = {
      running_to_cycling: 1.3,
      running_to_swimming: 0.8,
    ...
    const zoneAdjustments = {
      running_to_cycling: { Z1: 0.05, Z2: 0.03, Z3: 0.0, Z4: -0.05, Z5: -0.1 },
      running_to_swimming: { Z1: -0.05, Z2: 0.0, Z3: 0.0, Z4: 0.05, Z5: 0.1 },
      cycling_to_running: { Z1: -0.05, Z2: -0.03, Z3: 0.0, Z4: 0.05, Z5: 0.1 },
      cycling_to_swimming: { Z1: -0.1, Z2: -0.05, Z3: 0.0, Z4: 0.05, Z5: 0.15 },
      swimming_to_running: { Z1: 0.05, Z2: 0.0, Z3: 0.0, Z4: -0.05, Z5: -0.1 },
      swimming_to_cycling: { Z1: 0.1, Z2: 0.05, Z3: 0.0, Z4: -0.05, Z5: -0.15 },
    };
  ...
  checkAdaptationCompatibility(sourceAdaptation, targetAdaptation) {
    ...
    if (normalizedSource === normalizedTarget) {
      return { compatible: true, match: 'exact', confidence_bonus: 0.1 };
    }
    ...
    const isCompatible = compatible.some(
      adaptation => normalizedTarget.includes(adaptation) || adaptation.includes(normalizedTarget)
    );
  ...
  calculateConfidence(params) {
    ...
    const zonePenalty = { Z1: 0, Z2: 0, Z3: 0, Z4: -0.05, Z5: -0.1 };
...
class SubstitutionEngine {
  ...
  async suggest_substitutions(planned_session, target_modality, user_context) {
    ...
    const validCandidates = scaledCandidates.filter(c => {
      const durationCheck = EquivalenceRules.validateDurationLimits(
        sessionAnalysis.primary_zone,
        c.scaled_duration
      );
      ...
      if (adaptationCheck.compatible) {
        ...
        if (
          (!workout.equipment_required ||
            workout.equipment_required.every(eq => userEquipment.includes(eq))) &&
...
EquivalenceRules.validateDurationLimits = (zone, duration) => {
  ...
  if (duration < limit.min) {
    return {
      valid: false,
      reason: `Duration ${duration}min below minimum ${limit.min}min for ${zone}`,
    };
  }
  if (duration > limit.max) {
    return {
      valid: false,
      reason: `Duration ${duration}min exceeds maximum ${limit.max}min for ${zone}`,
    };
  }
...
exports.handler = withErrorHandling(async (event, _context) => {
  ...
  try {
        ...
        stats: {
          ...
          avg_load_variance:
            substitutions.length > 0
              ? Math.round(
                  (substitutions.reduce((sum, s) => sum + s.load_variance_percentage, 0) /
          ...
          avg_confidence:
            substitutions.length > 0
              ? Math.round(
                  (substitutions.reduce((sum, s) => sum + s.confidence_score, 0) /
                    substitutions.length) *
                    100
                ) / 100
              : 0,
    ...
    return createResponse(500, {
      error: 'Internal server error',
      message:
        process.env.NODE_ENV === 'development' ? error.message : 'Substitution generation failed',
    });
  }
});
