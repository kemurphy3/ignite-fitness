...
function calculateBaselines(metrics) {
    ...
    Object.entries(grouped).forEach(([type, typeMetrics]) => {
        const values = typeMetrics.map(m => {
            switch (type) {
                case 'lcp': return m.lcp_value;
                case 'fid': return m.fid_value;
                case 'cls': return m.cls_value;
                case 'ttfb': return m.ttfb_value;
                default: return null;
            }
        }).filter(v => v !== null);
...
function getMetricValue(metric) {
    switch (metric.metric_type) {
        case 'lcp': return metric.lcp_value;
        case 'fid': return metric.fid_value;
        case 'cls': return metric.cls_value;
        case 'ttfb': return metric.ttfb_value;
        default: return 0;
    }
}
