...
class SecurityAudit {
  ...
  async testTokenMasking() {
    ...
    try {
      ...
      if (
        maskedObject.access_token !== testObject.access_token &&
      ...
      if (maskingTestsPassed >= testTokens.length) {
        ...
        this.results.token_masking.details.push(
          `Only ${maskingTestsPassed}/${testTokens.length} tests passed`
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (fs.existsSync(revocationFile)) {
        ...
        if (securityChecksPassed === securityChecks.length) {
          ...
          this.results.token_revocation.details.push(
            `Only ${securityChecksPassed}/${securityChecks.length} security checks passed`
          );
        }
      } else {
    ...
    try {
      ...
      if (validation.isValid && validation.entropy >= 256) {
        ...
        if (weakSecretTestsPassed === weakSecrets.length) {
          ...
          this.results.jwt_validation.details.push(
            `Only ${weakSecretTestsPassed}/${weakSecrets.length} weak secret tests passed`
          );
        }
      } else {
