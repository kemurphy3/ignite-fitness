...
class TransactionRollbackManager {
  ...
  async executeRollback(transactionId) {
    ...
    this.logger.info(
      `Starting rollback for transaction ${transactionId} with ${transaction.actions.length} actions`
    );
  ...
  async compensateInsert(data) {
    try {
      const { error } = await this.supabase.from(data.table).delete().eq('id', data.id);
  ...
  async compensateDelete(data) {
    try {
      const { error } = await this.supabase.from(data.table).insert(data.originalRecord);
   ...
   */
  cleanupOldTransactions(maxAgeMs = 24 * 60 * 60 * 1000) {
    // 24 hours default
    const now = new Date();
