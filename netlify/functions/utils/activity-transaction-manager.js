...
class ActivityTransactionManager {
  ...
  async executeActivityDedupTransaction(normalized, userId, affectedDates) {
    ...
    try {
      ...
      console.log(
        `Starting transaction ${transactionId} for activity ${normalized.canonicalExternalId}`
      );
      ...
      let result;
      if (existingActivity) {
        result = await this.handleExistingActivityInTransaction(
          existingActivity,
          normalized,
          userId,
          affectedDates,
        ...
        if (likelyDuplicates.length > 0) {
          result = await this.handleLikelyDuplicateInTransaction(
            likelyDuplicates[0],
            normalized,
            userId,
            affectedDates,
          ...
          result = await this.handleNewActivityInTransaction(
            normalized,
            userId,
            affectedDates,
            transactionId
  ...
  async handleExistingActivityInTransaction(
    ...
    if (newRichness > existingRichness) {
      console.log(
        `Transaction ${transactionId}: Updating activity ${existing.id} with richer version`
      );
  ...
  async handleNewActivityInTransaction(normalized, userId, affectedDates, transactionId) {
    console.log(
      `Transaction ${transactionId}: Inserting new activity ${normalized.canonicalExternalId}`
    );
