...
async function testAdminAuthentication() {
  ...
  console.log('Invalid Token Result:', invalidResult.status, invalidResult.status === 401 ? '✅' : '❌');
...
async function testSessionsSeries(adminToken) {
  ...
  const seriesResult = await apiCall('admin-sessions-series?from=2024-01-01&to=2024-01-31&bucket=day&timezone=UTC', 'GET', null, {
    'Authorization': `Bearer ${adminToken}`
  });
  ...
  if (seriesResult.success) {
    ...
    if (data.series.length > 0) {
      ...
      const requiredFields = ['date', 'session_count', 'unique_users', 'completed_count', 'privacy_applied'];
  ...
  const invalidRangeResult = await apiCall('admin-sessions-series?from=2024-01-01&to=2023-01-01', 'GET', null, {
    'Authorization': `Bearer ${adminToken}`
  });
  
  console.log('Invalid Range Result:', invalidRangeResult.status, invalidRangeResult.status === 400 ? '✅' : '❌');
  ...
  const invalidTzResult = await apiCall('admin-sessions-series?from=2024-01-01&to=2024-01-31&timezone=Invalid/Zone', 'GET', null, {
    'Authorization': `Bearer ${adminToken}`
  });
  ...
  const weekResult = await apiCall('admin-sessions-series?from=2024-01-01&to=2024-01-31&bucket=week&timezone=America/New_York', 'GET', null, {
    'Authorization': `Bearer ${adminToken}`
  });
...
async function testSessionsByType(adminToken) {
  ...
  const dateRangeResult = await apiCall('admin-sessions-by-type?from=2024-01-01&to=2024-01-31', 'GET', null, {
    'Authorization': `Bearer ${adminToken}`
  });
...
async function testTopUsers(adminToken) {
  ...
  console.log('Top Sessions Result:', topSessionsResult.status, topSessionsResult.success ? '✅' : '❌');
  ...
  console.log('Top Duration Result:', topDurationResult.status, topDurationResult.success ? '✅' : '❌');
  ...
  console.log('Invalid Metric Result:', invalidMetricResult.status, invalidMetricResult.status === 400 ? '✅' : '❌');
  ...
  console.log('Invalid Limit Result:', invalidLimitResult.status, invalidLimitResult.status === 400 ? '✅' : '❌');
  ...
  if (topSessionsResult.success && topSessionsResult.data.data.next_cursor) {
    const paginationResult = await apiCall(`admin-users-top?metric=sessions&limit=5&cursor=${topSessionsResult.data.data.next_cursor}`, 'GET', null, {
      'Authorization': `Bearer ${adminToken}`
    });
    
    console.log('Pagination Result:', paginationResult.status, paginationResult.success ? '✅' : '❌');
  }
}
...
async function testAdminHealth(adminToken) {
  ...
  console.log('No Auth Health Result:', noAuthHealthResult.status, noAuthHealthResult.status === 401 ? '✅' : '❌');
}
...
async function testPrivacyProtection(adminToken) {
  ...
  const seriesResult = await apiCall('admin-sessions-series?from=2024-01-01&to=2024-01-31&bucket=day', 'GET', null, {
    'Authorization': `Bearer ${adminToken}`
  });
  ...
  if (topUsersResult.success) {
    ...
    if (data.users.length > 0) {
      const firstUser = data.users[0];
      const isHashed = firstUser.user_alias.startsWith('usr_') && firstUser.user_alias.length === 10;
...
async function testErrorHandling(adminToken) {
  ...
  const largeRangeResult = await apiCall('admin-sessions-series?from=2020-01-01&to=2024-01-01', 'GET', null, {
    'Authorization': `Bearer ${adminToken}`
  });
  
  console.log('Large Range Result:', largeRangeResult.status, largeRangeResult.status === 400 ? '✅' : '❌');
