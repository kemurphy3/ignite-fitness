...
async function testSMARTGoalFramework() {
    ...
    try {
        if (typeof window.GoalManager !== 'undefined') {
            ...
            if (completionResult.success || completionResult.error) { // Either success or expected error
                goalsHabitsTestResults.passed++;
...
async function testHabitFormation() {
    ...
    try {
        if (typeof window.HabitTracker !== 'undefined') {
            ...
            const achievementCheck = window.HabitTracker.checkAchievementCondition(
                testAchievement, 
                { current_streak: 7 }
            );
...
function displayGoalsHabitsTestResults() {
    ...
    const passRate = totalTests > 0 ? (goalsHabitsTestResults.passed / totalTests * 100).toFixed(1) : 0;
