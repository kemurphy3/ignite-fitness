...
async function checkLintingIssues() {
  ...
  try {
    ...
    if (consoleLogUsage) {
      ...
      ciCheckResults.linting.issues.push(
...
function displayCICheckResults() {
  ...
  const securityPassRate =
    securityTotal > 0 ? ((ciCheckResults.security.passed / securityTotal) * 100).toFixed(1) : 0;
  console.log(
    `🔒 Security: ${ciCheckResults.security.passed}/${securityTotal} (${securityPassRate}%)`
  );
  ...
  const lintingPassRate =
    lintingTotal > 0 ? ((ciCheckResults.linting.passed / lintingTotal) * 100).toFixed(1) : 0;
  ...
  const performancePassRate =
    performanceTotal > 0
      ? ((ciCheckResults.performance.passed / performanceTotal) * 100).toFixed(1)
      : 0;
  console.log(
    `⚡ Performance: ${ciCheckResults.performance.passed}/${performanceTotal} (${performancePassRate}%)`
  );
  ...
  const buildPassRate =
    buildTotal > 0 ? ((ciCheckResults.build.passed / buildTotal) * 100).toFixed(1) : 0;
  ...
  return {
    security: {
      passed: ciCheckResults.security.passed,
      failed: ciCheckResults.security.failed,
      passRate: securityPassRate,
    },
    linting: {
      passed: ciCheckResults.linting.passed,
      failed: ciCheckResults.linting.failed,
      passRate: lintingPassRate,
    },
    performance: {
      passed: ciCheckResults.performance.passed,
      failed: ciCheckResults.performance.failed,
      passRate: performancePassRate,
    },
    build: {
      passed: ciCheckResults.build.passed,
      failed: ciCheckResults.build.failed,
      passRate: buildPassRate,
    },
