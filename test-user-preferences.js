...
async function testGetPreferences() {
  ...
  if (getResult.success) {
    ...
    const hasDefaults = data.units === 'imperial' && 
  ...
  console.log('Non-existent User Result:', nonExistentResult.status, nonExistentResult.status === 403 ? '✅' : '❌');
...
async function testPatchPreferences(token) {
  ...
  const updateAllResult = await apiCall('users-preferences-patch', 'PATCH', {
  ...
  console.log('Update All Result:', updateAllResult.status, updateAllResult.status === 204 ? '✅' : '❌');

  // Test 2: Update partial preferences
  console.log('2. Testing update partial preferences...');
  const updatePartialResult = await apiCall('users-preferences-patch', 'PATCH', {
  ...
  console.log('Update Partial Result:', updatePartialResult.status, updatePartialResult.status === 204 ? '✅' : '❌');

  // Test 3: Empty body (no-op)
  console.log('3. Testing empty body (no-op)...');
  const emptyBodyResult = await apiCall('users-preferences-patch', 'PATCH', {}, {
    'Authorization': `Bearer ${token}`
  });
  
  console.log('Empty Body Result:', emptyBodyResult.status, emptyBodyResult.status === 204 ? '✅' : '❌');
  ...
  console.log('Large Body Result:', largeBodyResult.status, largeBodyResult.status === 400 ? '✅' : '❌');
}
...
async function testValidation() {
  ...
  const invalidTzResult = await apiCall('users-preferences-patch', 'PATCH', {
    timezone: 'Invalid/Zone'
  }, {
    'Authorization': `Bearer ${token}`
  });
  
  console.log('Invalid Timezone Result:', invalidTzResult.status, invalidTzResult.status === 400 ? '✅' : '❌');
  ...
  const invalidUnitsResult = await apiCall('users-preferences-patch', 'PATCH', {
    units: 'kilometers'
  }, {
    'Authorization': `Bearer ${token}`
  });
  
  console.log('Invalid Units Result:', invalidUnitsResult.status, invalidUnitsResult.status === 400 ? '✅' : '❌');

  // Test 3: Invalid sleep goal
  console.log('3. Testing invalid sleep goal...');
  const invalidSleepResult = await apiCall('users-preferences-patch', 'PATCH', {
    sleep_goal_hours: 25
  }, {
    'Authorization': `Bearer ${token}`
  });
  
  console.log('Invalid Sleep Goal Result:', invalidSleepResult.status, invalidSleepResult.status === 400 ? '✅' : '❌');

  // Test 4: Invalid workout goal
  console.log('4. Testing invalid workout goal...');
  const invalidWorkoutResult = await apiCall('users-preferences-patch', 'PATCH', {
    workout_goal_per_week: -1
  }, {
    'Authorization': `Bearer ${token}`
  });
  
  console.log('Invalid Workout Goal Result:', invalidWorkoutResult.status, invalidWorkoutResult.status === 400 ? '✅' : '❌');

  // Test 5: Invalid theme
  console.log('5. Testing invalid theme...');
  const invalidThemeResult = await apiCall('users-preferences-patch', 'PATCH', {
    theme: 'purple'
  }, {
    'Authorization': `Bearer ${token}`
  });
  
  console.log('Invalid Theme Result:', invalidThemeResult.status, invalidThemeResult.status === 400 ? '✅' : '❌');

  // Test 6: Valid timezone
  console.log('6. Testing valid timezone...');
  const validTzResult = await apiCall('users-preferences-patch', 'PATCH', {
    timezone: 'America/Denver'
  }, {
    'Authorization': `Bearer ${token}`
  });
  
  console.log('Valid Timezone Result:', validTzResult.status, validTzResult.status === 204 ? '✅' : '❌');
}
...
async function testCoercion() {
  ...
  const stringBoolResult = await apiCall('users-preferences-patch', 'PATCH', {
    notifications_enabled: 'true'
  }, {
    'Authorization': `Bearer ${token}`
  });
  
  console.log('String Bool Result:', stringBoolResult.status, stringBoolResult.status === 204 ? '✅' : '❌');

  // Test 2: Case insensitive units
  console.log('2. Testing case insensitive units...');
  const caseInsensitiveResult = await apiCall('users-preferences-patch', 'PATCH', {
    units: 'METRIC'
  }, {
    'Authorization': `Bearer ${token}`
  });
  
  console.log('Case Insensitive Result:', caseInsensitiveResult.status, caseInsensitiveResult.status === 204 ? '✅' : '❌');

  // Test 3: Decimal sleep goal rounding
  console.log('3. Testing decimal sleep goal rounding...');
  const decimalSleepResult = await apiCall('users-preferences-patch', 'PATCH', {
    sleep_goal_hours: 7.55
  }, {
    'Authorization': `Bearer ${token}`
  });
  
  console.log('Decimal Sleep Result:', decimalSleepResult.status, decimalSleepResult.status === 204 ? '✅' : '❌');

  // Test 4: Case insensitive theme
  console.log('4. Testing case insensitive theme...');
  const caseInsensitiveThemeResult = await apiCall('users-preferences-patch', 'PATCH', {
    theme: 'DARK'
  }, {
    'Authorization': `Bearer ${token}`
  });
  
  console.log('Case Insensitive Theme Result:', caseInsensitiveThemeResult.status, caseInsensitiveThemeResult.status === 204 ? '✅' : '❌');
}
...
async function testUnknownFields() {
  ...
  const unknownFieldsResult = await apiCall('users-preferences-patch', 'PATCH', {
  ...
  console.log('Unknown Fields Result:', unknownFieldsResult.status, unknownFieldsResult.status === 204 ? '✅' : '❌');
}
...
async function testConcurrentUpdates() {
  ...
  const update1 = apiCall('users-preferences-patch', 'PATCH', {
    timezone: 'America/New_York',
    theme: 'dark'
  }, {
    'Authorization': `Bearer ${token}`
  ...
  const update2 = apiCall('users-preferences-patch', 'PATCH', {
    units: 'metric',
...
async function testPersistence() {
  ...
  const updateResult = await apiCall('users-preferences-patch', 'PATCH', {
  ...
  if (getResult.success) {
    const data = getResult.data;
    const persistenceCheck = data.timezone === 'Europe/Berlin' &&
                           data.units === 'metric' &&
