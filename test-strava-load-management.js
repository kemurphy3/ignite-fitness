...
async function testStravaProcessor() {
  ...
  try {
    if (typeof window.StravaProcessor !== 'undefined') {
      ...
      const importResult = await window.StravaProcessor.importActivities(
...
async function testLoadCalculator() {
  ...
  try {
    if (typeof window.LoadCalculator !== 'undefined') {
      ...
      const weeklyLoad = window.LoadCalculator.calculateWeeklyLoad(
      ...
      const sessionLoad = window.LoadCalculator.calculateSessionLoad(
        STRAVA_LOAD_TEST_CONFIG.testSessions[0]
      );
      ...
      // Test recovery debt calculation
      const recoveryDebt = window.LoadCalculator.calculateRecoveryDebt(
        STRAVA_LOAD_TEST_CONFIG.testActivities
      );
...
async function testWorkoutAdjustments() {
  ...
  try {
    if (typeof window.StravaProcessor !== 'undefined') {
      ...
      const highIntensityAdjustment =
        window.StravaProcessor.suggestAdjustment(highIntensityActivity);
      ...
      stravaLoadTestResults.failed++;
      stravaLoadTestResults.errors.push(
        'StravaProcessor not available for workout adjustment testing'
      );
    }
  } catch (error) {
    stravaLoadTestResults.failed++;
...
function displayStravaLoadTestResults() {
  ...
  const passRate =
    totalTests > 0 ? ((stravaLoadTestResults.passed / totalTests) * 100).toFixed(1) : 0;
